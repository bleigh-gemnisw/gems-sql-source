...
#End Region
  ...
  Private Sub FrmFA002C_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    myFAAQUMT = New FAAQUMT.MyData()
    myFAAQUMT.MyDBConn = myDBConnect
    MyFrmFA002.TBarNew.Enabled = False
    MyFrmFA002.TBarSave.Enabled = True
    If WrkCode <> "" Then
      MyFrmFA002.TBarDelete.Enabled = True
      MyUtils.SetTxtReadOnly(TxtCode)
    End If
    MyFrmFA002.TBarPrint.Enabled = False
    myFAAQUMT.GetOneRecordP(WrkCode)
    TxtCode.Text = WrkCode
    If myFAAQUMT.RecordNotFound Then Exit Sub
    If s_chg = False And s_full = False Then    '#sec
      MyFrmFA002.TBarSave.Visible = False
    End If
    With myFAAQUMT
      TxtDesc.Text = Trim(._AQDESC)
    End With
  End Sub
  Private Sub FrmFA002C_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Activated
  MyFrmFA002.SbpScreen.Text = "FA002C"
  MyUtils.CenterForm(Me.ParentForm, Me)
End Sub
Private Sub FrmFA002C_Closed(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Closed
  MyFrmFA002.TBarNew.Enabled = True
  MyFrmFA002.TBarDelete.Enabled = False
  MyFrmFA002.TBarSave.Enabled = False
  MyFrmFA002.TBarPrint.Enabled = False
  MyFrmFA002.TBarSave.Visible = True   '#sec
  MyFrmFA002B.FormatGrid()
  MyFrmFA002B.Show()
End Sub
Public Sub DeleteData(ByRef WrkCancel As Boolean)
  ...
  End If
  WrkCancel = False
	myFAAQUMT.DeleteOneRecordP()
  Me.Close()
End Sub
Public Sub SaveData()
	Dim ErrorField(25) As String
	Dim ErrorMsg(25) As String
	myFAAQUMT.GetOneRecordP(TxtCode.Text)
	If WrkCode = "" Then
	...
	If WrkCode <> "" Then
		...
		If IsNothing(ErrorMsg(0)) Then
			myFAAQUMT.UpdateOneRecordP()
		Else
			ShowError(ErrorField, ErrorMsg)
			Exit Sub
		End If
	Else
		myFAAQUMT._AQCODE = TxtCode.Text
		MovetoFile()
		EditChecks(ErrorField, ErrorMsg)
		If IsNothing(ErrorMsg(0)) Then
			myFAAQUMT.AddOneRecordP()
		Else
			ShowError(ErrorField, ErrorMsg)
			Exit Sub
		End If
