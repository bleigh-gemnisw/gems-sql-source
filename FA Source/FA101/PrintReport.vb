...
Module PrintReport
  ...
  Dim WrkAsTypeCode(100) As String
  Dim WrkAsTypeDesc(100) As String
  Dim WrkBldgCode(100) As String
  Dim WrkBldgDesc(100) As String
  Dim WrkEqupCode(100) As String
  Dim WrkEqupDesc(100) As String
...
Done:
  ...
  Private Sub BuildDS()
    ...
    With myTable
      ...
      .Columns.Add("BldgDesc", Type.GetType("System.String"))
      .Columns.Add("AsTypeDesc", Type.GetType("System.String"))
      .Columns.Add("EqupDesc", Type.GetType("System.String"))
    End With
  ...
  Private Sub GetDetail()
    ...
    End If
    If MyFrmFA101B.RbFundBusiness.Checked Then
      WrkQry = WrkQry & WrkAnd & "FAGOV<>'Y'"
    End If
    If MyFrmFA101B.RbFundGovernment.Checked Then
      WrkQry = WrkQry & WrkAnd & "FAGOV='Y'"
    End If
    If MyFrmFA101B.RbDeprYes.Checked Then
      WrkQry = WrkQry & WrkAnd & "FANDEP<>'Y'"
    End If
    If MyFrmFA101B.RbDeprNon.Checked Then
      WrkQry = WrkQry & WrkAnd & "FANDEP='Y'"
    End If
    If MyFrmFA101B.RbAssetsReport.Checked Then
      WrkQry = WrkQry & WrkAnd & "FANREP<>'Y'"
    End If
    If MyFrmFA101B.RbAssetsNon.Checked Then
      WrkQry = WrkQry & WrkAnd & "FANREP='Y'"
    End If
    ...
    For I = 0 To (DsFAMSTR.Tables(0).Rows.Count - 1)
      With DsFAMSTR.Tables(0).Rows(I)
        ...
        End If
        dr.Item("bldgdesc") = LookupBldgDesc(.Item("fablcd"))
        dr.Item("astypedesc") = LookupAsTypeDesc(.Item("faascd"))
        dr.Item("equpdesc") = LookupEqupDesc(.Item("faeqcd"))
      End With
...
NextRec:
  ...
  Private Function LookupAcqDesc(ByVal Code As String) As String
    ...
    Return ""
  End Function
  Private Sub BufferAsTypeDesc()
    Dim I As Integer

    Dim myFAASTYPE As FAASTYPE.MyData
    Dim dsFAASTYPE As DataSet = New DataSet

    myFAASTYPE = New FAASTYPE.MyData()
    myFAASTYPE.MyDBConn = myDBConnect

    dsFAASTYPE = myFAASTYPE.PosData("")
    For I = 0 To dsFAASTYPE.Tables(0).Rows.Count - 1
      With dsFAASTYPE.Tables(0).Rows(I)
        WrkAsTypeCode(I) = .Item("ascode")
        WrkAsTypeDesc(I) = .Item("asdesc")
      End With
    Next

  End Sub
  Private Function LookupAsTypeDesc(ByVal Code As String) As String
    Dim I As Integer
    Dim WrkDesc As String

    For I = 0 To WrkAsTypeCode.GetUpperBound(0)
      If WrkAsTypeCode(I) Is Nothing Then
        Return ""
      End If
      If Code = WrkAsTypeCode(I) Then
        WrkDesc = WrkAsTypeDesc(I)
        Return WrkDesc
      End If
    Next
    ...
    Return ""
  End Function
  Private Sub BufferBldgDesc()
    Dim I As Integer

    Dim myFABLDG As FABLDG.MyData
    Dim dsFABLDG As DataSet = New DataSet

    myFABLDG = New FABLDG.MyData()
    myFABLDG.MyDBConn = myDBConnect

    dsFABLDG = myFABLDG.PosData("")
    For I = 0 To dsFABLDG.Tables(0).Rows.Count - 1
      With dsFABLDG.Tables(0).Rows(I)
        WrkBldgCode(I) = .Item("blcode")
        WrkBldgDesc(I) = .Item("bldesc")
      End With
    Next

  End Sub
  Private Function LookupBldgDesc(ByVal Code As String) As String
    Dim I As Integer
    Dim WrkDesc As String

    For I = 0 To WrkBldgCode.GetUpperBound(0)
      If WrkBldgCode(I) Is Nothing Then
        Return ""
      End If
      If Code = WrkBldgCode(I) Then
        WrkDesc = WrkBldgDesc(I)
        Return WrkDesc
      End If
    Next
    ...
    Return ""
  End Function
  Private Sub BufferEqupDesc()
    Dim I As Integer

    Dim myFAEQUP As FAEQUP.MyData
    Dim dsFAEQUP As DataSet = New DataSet

    myFAEQUP = New FAEQUP.MyData()
    myFAEQUP.MyDBConn = myDBConnect

    dsFAEQUP = myFAEQUP.PosData("")
    For I = 0 To dsFAEQUP.Tables(0).Rows.Count - 1
      With dsFAEQUP.Tables(0).Rows(I)
        WrkEqupCode(I) = .Item("eqcode")
        WrkEqupDesc(I) = .Item("eqdesc")
      End With
    Next

  End Sub
  Private Function LookupEqupDesc(ByVal Code As String) As String
    Dim I As Integer
    Dim WrkDesc As String

    For I = 0 To WrkEqupCode.GetUpperBound(0)
      If WrkEqupCode(I) Is Nothing Then
        Return ""
      End If
      If Code = WrkEqupCode(I) Then
        WrkDesc = WrkEqupDesc(I)
        Return WrkDesc
      End If
    Next
