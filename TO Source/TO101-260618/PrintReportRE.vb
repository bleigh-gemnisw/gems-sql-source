...
Module PrintReportRE
  ...
  Private Sub GetDetail()
    ...
    Dim WrkOExPhaseIn As Long
    Dim WrkPExPhaseIn As Long
    ...
    For I = 0 To (DsTXREAL.Tables(0).Rows.Count - 1)
        ...
        Else
          ...
          If WrkBTR And .Item("btr") <> 0 Then
            If Not WrkFrozenFile Then
              ...
              End If
            Else
              myTXBTRC.GetOneRecordP(WrkListNo, WrkType)
        ...
        If WrkPhaseIn Then
          ...
          With myTXPHIN
            If Not .RecordNotFound Then
              WrkFullGross = ._ORIGRS + ._FULGRS
              If ._FULGRS > 0 Then
                myTXREALC.GetOneRecordP(WrkListNo)
                With myTXREALC
                  If Not .RecordNotFound Then
                    If WrkAssCode(0) >= 11 And WrkAssCode(0) <= 19 Or WrkAssCode(0) = 100 Then
                      If WrkBTR Then
                        WrkOExPhaseIn = WrkFullGross - ._GROSS - ._BTR
                      Else
                        WrkOExPhaseIn = WrkFullGross - ._GROSS
                      End If
                    Else
                      If WrkBTR Then
                        WrkPExPhaseIn = WrkFullGross - ._GROSS - ._BTR
                      Else
                        WrkPExPhaseIn = WrkFullGross - ._GROSS
                      End If
                    End If
                    'Hard code phase in for year 1 
                    WrkGross(0) = WrkGross(0) + (myTXPHIN._CAPA1 * 4) + (myTXPHIN._ADJA1 * 4)
                    WrkGross(1) = WrkGross(1) + (myTXPHIN._CAPA2 * 4) + (myTXPHIN._ADJA2 * 4)
                    WrkGross(2) = WrkGross(2) + (myTXPHIN._CAPA3 * 4) + (myTXPHIN._ADJA3 * 4)
                    WrkGross(3) = WrkGross(3) + (myTXPHIN._CAPA4 * 4) + (myTXPHIN._ADJA4 * 4)
                    WrkGross(4) = WrkGross(4) + (myTXPHIN._CAPA5 * 4) + (myTXPHIN._ADJA5 * 4)
                    WrkGross(5) = WrkGross(5) + (myTXPHIN._CAPA6 * 4) + (myTXPHIN._ADJA6 * 4)
                    WrkGross(6) = WrkGross(6) + (myTXPHIN._CAPA7 * 4) + (myTXPHIN._ADJA7 * 4)
                  End If
                End With
              End If
            End If
      ...
      If WrkPhaseIn Then
        WrkTotExRE = WrkTotExRE + WrkOExPhaseIn + WrkPExPhaseIn
        If WrkOExPhaseIn > 0 Then
          K = LookupWrkTExCode("O")
          WrkTExCode(K) = "O"
          WrkTExRECount(K) = WrkTExRECount(K) + 1
          WrkTExRE(K) = WrkTExRE(K) + WrkOExPhaseIn
        End If
        If WrkPExPhaseIn > 0 Then
          K = LookupWrkTExCode("P")
          WrkTExCode(K) = "P"
          WrkTExRECount(K) = WrkTExRECount(K) + 1
          WrkTExRE(K) = WrkTExRE(K) + WrkPExPhaseIn
        End If
      End If
