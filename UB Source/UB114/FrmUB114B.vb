...
#End Region
  ...
  Public Sub ShowGrid()
    ...
    If RbName.Checked Then
      'UB114 only lists customers that have a meter number.
      'Use UTCUSTQ so the meter condition is applied BEFORE TOP 50.
      WrkName = SqlText(TxtPos.Text)
      WrkQry = "LTRIM(RTRIM(ISNULL(CUMETN,''))) <> ''"

      If WrkListNo > 0 Then
        WrkQry = WrkQry & " and (CUNAM1 > '" & WrkName & "'" _
          & " or (CUNAM1 = '" & WrkName & "' and CUACCT > " & WrkListNo & "))"
      ElseIf WrkName <> String.Empty Then
        WrkQry = WrkQry & " and CUNAM1 >= '" & WrkName & "'"
      End If

      WrkSort = "CUNAM1, CUACCT"
      ds = myUTCUSTQ.GetQry(WrkSort, WrkQry, 50)
    ...
    ElseIf RbLoc.Checked Then
      ...
      WrkLoc = SqlText(TxtPos.Text)
      WrkPosNo = SqlText(WrkPosNo)
      WrkQry = "LTRIM(RTRIM(ISNULL(CUMETN,''))) <> ''"

      If WrkListNo > 0 Then
        WrkQry = WrkQry & " and (CULOC > '" & WrkLoc & "'" _
          & " or (CULOC = '" & WrkLoc & "' and [CULOC#] > '" & WrkPosNo & "')" _
          & " or (CULOC = '" & WrkLoc & "' and [CULOC#] = '" & WrkPosNo _
          & "' and CUACCT > " & WrkListNo & "))"
      ElseIf WrkLoc <> String.Empty OrElse Trim(WrkPosNo) <> String.Empty Then
        WrkQry = WrkQry & " and (CULOC > '" & WrkLoc & "'" _
          & " or (CULOC = '" & WrkLoc & "' and [CULOC#] >= '" & WrkPosNo & "'))"
      End If

      WrkSort = "CULOC, [CULOC#], CUACCT"
      ds = myUTCUSTQ.GetQry(WrkSort, WrkQry, 50)

    ElseIf RbMeter.Checked Then
      'Keep Mark's existing meter-number index/query.
    ...
    If ds IsNot Nothing AndAlso ds.Tables.Count > 0 Then
      ...
      DataGrdView.CurrentCell = Nothing
    End If
    Windows.Forms.Cursor.Current = Cursors.Default
  End Sub

  Private Function SqlText(ByVal WrkValue As String) As String
    Return Trim(WrkValue).Replace("'", "''")
  End Function
  ...
  Private Function BuildMeteredDisplayTable(ByVal SourceTable As DataTable, ByVal SourceHasMeter As Boolean) As DataTable
    ...
    For Each SourceRow As DataRow In SourceTable.Rows
      ...
      If SourceTable.Columns.Contains("CUACCT") AndAlso SourceTable.Columns.Contains("CUMETN") Then
        'UTCUSTQ returns UTCUST fields directly.
        Acct = CInt(SourceRow("CUACCT"))
        MeterNo = Trim(Convert.ToString(SourceRow("CUMETN")))
        CustName = Trim(Convert.ToString(SourceRow("CUNAM1")))
        LocNo = Trim(Convert.ToString(SourceRow("CULOC#")))
        Location = Trim(Convert.ToString(SourceRow("CULOC")))
      ElseIf SourceHasMeter Then
        'Existing UTCUSTL1 meter-number query.
        Acct = CInt(SourceRow(0))
      ...
      Else
        'Safety fallback for the older name/location index layouts.
        Acct = CInt(SourceRow(0))
