...
Private Sub FrmComments_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
  ...
  If s_full = True Or s_add = True Or s_chg = True Then
    MyFrmUB114.TBarSave.Enabled = True
    MyFrmUB114.TBarSave.Visible = True
  End If

  myCOM = New TAXCOM.myData(myDBConnect)
  LblList.Text = WrkListNo
  LblName.Text = MyFrmUB114C.LblName.Text
...
Public Sub FormatText()
  ...
  ds = myCOM.Getcomments(WrkListNo, WrkType, WrkYear)
  WrkStr = ""
...
Private Sub FrmComments_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Activated
  MyFrmUB114.SbpScreen.Text = "Comments"
...
End Sub
  Public Sub SaveData()
    ...
    myCOM.DeleteKeyComment(WrkListNo, WrkType, WrkYear)
    WrkLen = Len(TxtComment.Text)
    WrkRecs = Math.Ceiling(WrkLen / 60)
    For I = 0 To WrkRecs - 1
      WrkPos = (I * 60) + 1
      With myCOM
        .GetOneRecordP(WrkListNo, WrkType, WrkYear, I + 1)
        ._LISTNO = WrkListNo
        ._TYPE = WrkType
        ._YEAR = WrkYear
        ._CSEQ = I + 1
