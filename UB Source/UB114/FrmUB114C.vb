...
#End Region
  ...
  Private Sub FrmUB114C_Closed(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Closed
    MyFrmUB114.TBarDelete.Enabled = False
    MyFrmUB114.TBarComments.Enabled = False
    MyFrmUB114.TBarAttach.Enabled = False
    MyFrmUB114.TBarAttach.Text = "Attachments"
  ...
  End Sub
  Public Sub SetToolbarState()
    Dim myCOM As TAXCOM.myData
    Dim dsCOM As DataSet
    Dim WrkAttachCount As Integer

    MyFrmUB114.TBarComments.Enabled = True
    MyFrmUB114.TBarAttach.Enabled = True

    myCOM = New TAXCOM.myData(myDBConnect)
    dsCOM = myCOM.Getcomments(WrkListNo, "U", 0)
    MyFrmUB114.TBarComments.ImageKey = ""
    If dsCOM IsNot Nothing AndAlso dsCOM.Tables.Count > 0 AndAlso dsCOM.Tables(0).Rows.Count > 0 Then
      MyFrmUB114.TBarComments.ImageKey = "comment_24.png"
    End If

    'Share the same account-level attachments used by UB102.
    WrkAttachCount = GetAttachcount("UB102", MyUtils.CnvSng(WrkListNo))
    MyFrmUB114.TBarAttach.Text = WrkAttachCount & " Attachment(s)"
    SetDeleteState()
  End Sub
