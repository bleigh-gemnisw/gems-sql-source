...
Module PrintReport
  ...
  myUTCUSTQ = New UTCUSTQ.mydata(MyDBConnect)
    ...
    With MyFrmUB234B
      ...
      WrkSize = Trim(.TxtMeterSize.Text)
      WrkPropCat = Trim(.TxtPropCat.Text)
    End With
...
Else
 ...
 End If
    ...
    If WrkSize <> String.Empty Then
      If WrkQry = String.Empty Then
        WrkQry = "cumsiz = '" & WrkSize & "'"
      Else
        WrkQry = WrkQry & WrkAnd & "cumsiz = '" & WrkSize & "'"
      End If
    End If
    If WrkPropCat <> String.Empty Then
      If WrkQry = String.Empty Then
        WrkQry = "cupcat = '" & WrkPropCat & "'"
      Else
        WrkQry = WrkQry & WrkAnd & "cupcat = '" & WrkPropCat & "'"
      End If
    End If
...
ReadNext:
  ...
  If Not myUTCUSTQ.IsEOF Then
      With myUTCUSTQ
        ...
        ' 9/9/24 add meter desc
        If ._CUMSIZ > "" Then
          dr.Item("mtdesc") = GetMeterDesc(WrkUBType, ._CUMSIZ)
        Else
          dr.Item("mtdesc") = ""
        End If
...
Friend Sub BuildDS(ByRef ds As DataSet)
  ...
  With myTable
    ...
    .Columns.Add("addr2", Type.GetType("System.String"))
      ...
      .Columns.Add("mtdesc", Type.GetType("System.String"))
    End With
...
End Sub
  Private Function GetRateCode(ByVal WrkListno As Integer, ByVal WrkUBType As String) As String
    ...
    End With
  End Function
  Private Function GetMeterDesc(ByVal Wrkmtype As String, ByVal WrkmSize As String) As String
    GetMeterDesc = ""
    myUTMETER.GetOneRecordP(Wrkmtype, WrkmSize)
    If myUTCUSTRT.RecordNotFound Then Exit Function

    With myUTMETER
      GetMeterDesc = ._MTDESC
    End With
  End Function
End Module
