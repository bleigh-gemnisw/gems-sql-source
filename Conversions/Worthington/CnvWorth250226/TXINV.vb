...
#Region "Methods: File Access Routines"
  ...
  Public Sub GetOneRecordP(ByVal Wrklistno As Integer, ByVal Wrkyear As Integer, ByVal Wrktype As String)
    ...
    End Try
  End Sub
  Public Sub GetAssrListNo(ByVal Wrklistno As Integer, ByVal Wrkyear As Integer)
    Dim objCommand As SqlCommand
    Dim ds As DataSet = New DataSet

    RecordNotFound = False
    ErrMsg = ""
    StrSQL = "Select * from " & cFilename & " where rlst = " & Wrklistno & " and year = " & Wrkyear
    Try
      objCommand = New SqlCommand(StrSQL, Conn)
      'Fill the dataset with the data
      da = New SqlDataAdapter
    ...
    End Try
  End Sub
  Public Sub FindListNo(ByVal Wrklistno As Integer)
    Dim objCommand As SqlCommand
    Dim ds As DataSet = New DataSet
  ...
  Public Sub InsertOneRecordP()
    ...
    _CHDATE & cnn & _CHTIME & cnn & _DEFERT & cnn & _DEFER1 & cnn & _DEFER2 & cnn & _DEFER3 & cnn & _DEFER4 & ")"
    Try
      objCommand = New SqlCommand(StrSQL, Conn)
      da = New SqlDataAdapter
