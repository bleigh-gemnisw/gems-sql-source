...
Public Class FrmMain
  ...
  Dim WrkLocID(2500) As Integer
  Dim WrkCustID(2500) As Integer
  Dim WrkService(2500) As String 'S=Sewer/W=Water=B=Both
  Dim WrkBillFreq(2500) As String 'M=Monthly/Q=Quarterly/P=Meter Pit
  Dim WrkMeterSize(2500) As String
  Dim WrkMIU(2500) As String
...
NextLine:
    ...
    WrkListNo = CnvSng(RecArray(2)) 'Location ID
    WrkDate = RecArray(6)
...
Process:
    ...
    End If
    WrkMonthly = False
    K = LookupLocID(WrkListNo)
    If K >= 0 Then
      If WrkBillFreq(K) = "M" Then
        WrkMonthly = True
      End If
    End If
...
NextLine:
    ...
    If Not WrkMonthly Then
      If WrkService(K) = "B" Or WrkService(K) = "S" Then
        With MyUTCUSTRT
        ...
        End With
      End If
      If WrkService(K) = "B" Or WrkService(K) = "W" Then
        With MyUTCUSTRT
...
Process:
    WrkListNo = CnvSng(RecArray(0)) 'Location ID
    WrkCode = ""
...
ShowPct:
  ...
  '    GoTo NextLine
  '  End Sub
  Private Sub UpdateUT()
    Dim WrkStream As FileStream = New FileStream(MyAppSettings.FileUT4, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)
    Dim sr As StreamReader = New StreamReader(WrkStream)
    Dim WrkFileSize As Integer
    Dim RecArray As String()
    Dim WrkPct As Decimal
...
Process:
    I = I + strBuffer.Length
    RecArray = Parse(strBuffer, ",")
    If Trim(RecArray(6)) = "A" Then
      WrkLocID(J) = RecArray(0)
      WrkCustID(J) = RecArray(1)
      WrkService(J) = Trim(RecArray(7))
      WrkBillFreq(J) = Trim(RecArray(8))
      WrkMeterSize(J) = Trim(RecArray(9))
      WrkMIU(J) = RecArray(21)
      J = J + 1
    End If
    GoTo NextLine
  End Sub
  Public Function LookupCustID(ByVal CustID As Integer) As Array
    Dim aLoc(10) As Integer
    Dim I As Integer
    Dim J As Integer
  ...
  Public Function LookupLocID(ByVal LocID As Integer) As Integer
    ...
    For I = 0 To WrkLocID.GetUpperBound(0)
      If Trim(WrkLocID(I)) = 0 Then
        Return -1
      End If
      If LocID = WrkLocID(I) Then
        Return I
      End If
    Next
    Return -1
  ...
  '  WrkCustNo = WrkCustID(K)
  '  Select Case WrkCustNo
