...
Public Class FrmMain
  ...
  Private Sub BtnConvert_Click(sender As Object, e As EventArgs) Handles BtnConvert.Click
    ...
    TxtErrorMsg.Text = TxtErrorMsg.Text & "NEWINV" & vbCrLf
    NewINV() 'Invoice file
    TxtErrorMsg.Text = TxtErrorMsg.Text & "NEWHST" & vbCrLf
    NewHST() 'History file
  ...
  Private Sub DoNotRun()
    ...
    'TxtErrorMsg.Text = TxtErrorMsg.Text & "Update meter" & vbCrLf
    'UpdateUTCUSTMeter()
    'TxtErrorMsg.Text = TxtErrorMsg.Text & "Missing meters" & vbCrLf
    'MissingMeters()
    'TxtErrorMsg.Text = TxtErrorMsg.Text & "Missing TXINV" & vbCrLf
    'MissingINV() 'Invoice file
    'TxtErrorMsg.Text = TxtErrorMsg.Text & "Missing TXHST" & vbCrLf
    'MissingHST() 'History file
    'TxtErrorMsg.Text = TxtErrorMsg.Text & "Adjust TXINV" & vbCrLf
    'AdjustINV() 'Invoice file
    ...
    'WriteLOGUT() 'UB Log
    'TxtErrorMsg.Text = TxtErrorMsg.Text & "Missing TXINV New" & vbCrLf
    'MissingINVNew() 'Invoice file
    'TxtErrorMsg.Text = TxtErrorMsg.Text & "Missing TXHST New" & vbCrLf
    'MissingHSTNew() 'History file
    'TxtErrorMsg.Text = TxtErrorMsg.Text & "MUNINV" & vbCrLf
    'MunINV() 'Invoice file
    'TxtErrorMsg.Text = TxtErrorMsg.Text & "MUNHST" & vbCrLf
    'MunHST() 'History file
  End Sub
...
ShowPct:
  ...
  Private Sub MissingMeters()
    ...
    dsgems = myDBConnectGEMS.RunQuery2("meterlist", "")
    For I = 0 To dsgems.Tables(0).Rows.Count - 1
      With MyUTCUST
        Counter = Counter + 1
        WrkMeter = dsgems.Tables(0).Rows(I).Item("meterno")
        ds = myDBConnect.RunQuery("utactcid", " where utacd_account='" & WrkMeter & "'")
        WrkMunACMKey = 0
        WrkMunACDKey = 0
        WrkName = ""
        Good = False
        For J = 0 To ds.Tables(0).Rows.Count - 1
          If ds.Tables(0).Rows(J).Item("utacd_stop_date") <> "9999-12-31 00:00:00.000" Then
            Continue For
          End If
          WrkMunCid = ds.Tables(0).Rows(J).Item("utacd_cid")
          ds2 = myDBConnect.RunQuery("ub_customers", "where a_customer=" & WrkMunCid)
          If ds2.Tables(0).Rows.Count = 0 Then
            GoTo ShowPct
          End If
          WrkName = Replace(ds2.Tables(0).Rows(0).Item("cs_name1"), ",", "")
          WrkMunACMKey = ds.Tables(0).Rows(J).Item("utacd_utacm_key")
          WrkMunACDKey = ds.Tables(0).Rows(J).Item("utacd_key")
          WrkMeter = ds.Tables(0).Rows(J).Item("utacd_account")
          ds3 = myDBConnect.RunQuery("utactmst", "where utacm_key=" & WrkMunACMKey)
        ...
        WrkLocNo = JustifyRight(ds3.Tables(0).Rows(0).Item("utacm_loc_no"), 7)
        WrkLoc = Trim(ds3.Tables(0).Rows(0).Item("utacm_loc_street")) & " " & Trim(ds3.Tables(0).Rows(0).Item("utacm_loc_str_typ"))
        WrkLocSuff = Trim(ds3.Tables(0).Rows(0).Item("utacm_loc_no_suff"))
        ...
        For J = 0 To dstemp.Tables(0).Rows.Count - 1
          ...
          WrkServiceCd = dstemp.Tables(0).Rows(J).Item("utsvm_serv_code")
        Next
        sw.WriteLine(WrkMeter & "," & WrkName & "," & WrkLocNo & " " & WrkLocSuff & " " & WrkLoc & "," & WrkServiceCd & "," & WrkRateCd)
      End With
...
ShowPct:
  ...
  Private Sub UpdateUTCUSTMeter()
    ...
    MyUTCUST = New UTCUST(myDBConnectGEMS.MyConn2)
    MyUTXREF = New UTXREF(myDBConnectGEMS.MyConn2)
    myDBConnectGEMS.DeleteRecords2("UTXREF")

    ds = myDBConnectGEMS.RunQuery2("utcust", "")
    For I = 0 To ds.Tables(0).Rows.Count - 1
      With MyUTCUST
        Counter = Counter + 1
        WrkAcct = ds.Tables(0).Rows(I).Item("cuacct")
        WrkMunCid = ds.Tables(0).Rows(I).Item("cucnt#")
        Array.Clear(WrkMeter, 0, 100)
        ds2 = myDBConnect.RunQuery("utactcid", "where utacd_cid=" & WrkMunCid & " order by utacd_account desc")
        If ds2.Tables(0).Rows.Count = 0 Then
          Continue For
        End If
        WrkMunACMKey = 0
        ...
        For J = 0 To ds2.Tables(0).Rows.Count - 1
          ...
          If ds3.Tables(0).Rows.Count > 0 Then
            ...
            If ds4.Tables(0).Rows.Count > 0 Then
              Good = True
              K = K + 1
              WrkMeter(K) = ds2.Tables(0).Rows(J).Item("utacd_account")
            End If
          End If
        Next
        ...
        .GetOneRecordP(WrkAcct)
        If K = 0 Then
          If Trim(WrkMeter(K)) <> Trim(._CUMETN) Then
            ._CUMETN = WrkMeter(K)
            .UpdateOneRecordP()
          End If
        Else
          ...
          For J = 0 To K
            With MyUTXREF
              .GetOneRecordP(WrkAcct, "", WrkMeter(J))
              ._CXACCT = WrkAcct
              ._CXCODE = ""
              ._CXUSE = ""
              ._CXREF = WrkMeter(J)
...
ReadNext:
  ...
  Private Sub WriteLOGUT()
    ...
    For I = 0 To ds.Tables(0).Rows.Count - 1
      ...
      WrkMeter = ds.Tables(0).Rows(I).Item("utacd_account")
      WrkMunCid = ds.Tables(0).Rows(I).Item("utacd_cid")
      ds2 = myDBConnectGEMS.RunQuery2("LOGUT", " where cumetn='" & WrkMeter & "' and cucnt#=" & WrkMunCid)
      If ds2.Tables(0).Rows.Count > 0 Then
        Continue For
      End If
...
ReadNext:
  ...
  Private Sub MissingHST()
    ...
    Dim WrkRecID As Integer
    MyTXHST = New TXHST(myDBConnectGEMS.MyConn2)
    ds = myDBConnectGEMS.RunQuery2("LOGUT", " where logcmt='New'")
    WrkFile = "TXHST"
    For I = 0 To ds.Tables(0).Rows.Count - 1
      With MyTXHST
        Counter = Counter + 1
...
NextRec:
  ...
  Private Sub MissingINVNew()
    ...
    MyTXINV = New TXINV(myDBConnectGEMS2.MyConn2)
    myDBConnectGEMS.OpenQry2("utcust", " where oid='Miss2'")
    WrkFile = "TXINV"
    WrkType = "D"
    WrkOldYear = 2012 '15 Years
...
ReadNext:
  ...
  Private Sub MissingHSTNew()
    ...
    ds = myDBConnectGEMS.RunQuery2("UTCUST", " where oid='Miss2'")
    WrkFile = "TXHST"
    For I = 0 To ds.Tables(0).Rows.Count - 1
      With MyTXHST
        Counter = Counter + 1
        ...
        For J = 0 To ds2.Tables(0).Rows.Count - 1
          ...
          If Not MyTXINV.RecordNotFound Then
            If Trim(MyTXINV._PRF) <> "Miss2" Then
              Continue For
            End If
          Else
            Continue For
...
NextRec:
  ...
  Private Sub AdjustINV()
    ...
    Dim WrkAdjust1 As Decimal
    Dim WrkAdjust2 As Decimal
    Dim WrkAdjust3 As Decimal
    Dim WrkAdjust4 As Decimal
    Dim WrkPct As Decimal
    Dim SavePct As Decimal
    Dim Counter As Decimal
    Dim I As Integer
      ...
      With MyTXINV
        ...
        For I = 0 To ds2.Tables(0).Rows.Count - 1
          ...
          If ._BALD <= 0 Then
            Continue For
          End If
          If ds2.Tables(0).Rows(I).Item("bh_unpd_bal") = 0 And ._BALD <= 0 Then
            Continue For
          End If
          WrkInstallNo = CalcInstallNo(myDBConnectGEMS.objReader.Item("cudst"), ds2.Tables(0).Rows(I).Item("bh_bill_date1"))
          WrkBillNo = ds2.Tables(0).Rows(I).Item("a_bill_number")
          ...
          WrkAdjust3 = 0
          WrkAdjust4 = 0
          ds3 = myDBConnect.RunQuery("ub_bill_detail", "where a_acct=" & WrkMunCid & " and a_bill_number=" & WrkBillNo & " and bd_original_amount>0 and bd_adjust_amount<>0")
          For J = 0 To ds3.Tables(0).Rows.Count - 1
            WrkAdjust = ds3.Tables(0).Rows(J).Item("bd_adjust_amount")
            Select Case WrkInstallNo
              Case 1
                WrkAdjust1 = WrkAdjust1 + WrkAdjust
              Case 2
                WrkAdjust2 = WrkAdjust2 + WrkAdjust
              Case 3
                WrkAdjust3 = WrkAdjust3 + WrkAdjust
              Case 4
                WrkAdjust4 = WrkAdjust4 + WrkAdjust
            End Select
          Next
          WrkAdjust = WrkAdjust1 + WrkAdjust2 + WrkAdjust3 + WrkAdjust4
          If WrkAdjust = 0 Then
            Continue For
          End If
          ._TAX1 = ._TAX1 + WrkAdjust1
          If ._TAX1 < 0 Then
            ._TAX1 = 0
          End If
          ._TAX2 = ._TAX2 + WrkAdjust2
          If ._TAX2 < 0 Then
            ._TAX2 = 0
          End If
          ._TX3RD = ._TX3RD + WrkAdjust3
          If ._TX3RD < 0 Then
            ._TX3RD = 0
          End If
          ._TX4TH = ._TX4TH + WrkAdjust4
          If ._TX4TH < 0 Then
            ._TX4TH = 0
          End If
          ._TAXT = ._TAX1 + ._TAX2 + ._TX3RD + ._TX4TH
          ._BALD = ._TAXT - ._PAYREC
          sw.WriteLine("TXINV," & ._LISTNo & "," & ._YEAR & "," & ._TAXT & "," & WrkAdjust)
          ._PRF = "Adj"
          .UpdateOneRecordP()
        Next
      End With
...
ReadNext:
    ...
    If Not myDBConnectGEMS.IsEOF Then
      With MyTXINV
        ...
        WrkListNo = myDBConnectGEMS.objReader.Item("cuacct")
        WrkMunAcct = Trim(myDBConnectGEMS.objReader.Item("cumetn"))
        WrkMunCid = CnvSng(myDBConnectGEMS.objReader.Item("cucnt#"))
        '        If WrkMunAcct <> "" And WrkMunCid > 0 Then
        '        ds2 = myDBConnect.RunQuery("ub_bill_header", "where a_bill_year>=" & WrkOldYear & " And a_acct=" & WrkMunCid & " And a_account='" & WrkMunAcct & "' and bh_bill_amount<>0")
        '       Else
        ds2 = myDBConnect.RunQuery("ub_bill_header", "where a_bill_year>=" & WrkOldYear & " And a_account='" & WrkMunAcct & "' and bh_bill_amount<>0")
        '      End If
        For I = 0 To ds2.Tables(0).Rows.Count - 1
          WrkYear = ds2.Tables(0).Rows(I).Item("a_bill_year")
          WrkMunCid = CnvSng(ds2.Tables(0).Rows(I).Item("a_acct"))
          .GetOneRecordP(WrkListNo, WrkYear, WrkType)
          WrkInstallNo = CalcInstallNo(myDBConnectGEMS.objReader.Item("cudst"), ds2.Tables(0).Rows(I).Item("bh_bill_date1"))
          ._ADD1 = myDBConnectGEMS.objReader.Item("cuadd1")
          ._ADD2 = myDBConnectGEMS.objReader.Item("cuadd2")
  ...
  Private Sub MunHST()
    ...
    MyTXHST = New TXHST(myDBConnectGEMS.MyConn2)
    MyUTCUST = New UTCUST(myDBConnectGEMS.MyConn2)
    ds = myDBConnectGEMS.RunQuery2("MUNINV", "")
    WrkFile = "MUNHST"
    WrkOldYear = 2026
    ...
    For I = 0 To ds.Tables(0).Rows.Count - 1
      With MyTXHST
        ...
        'ds2 = myDBConnect.RunQuery("ar_history_header", "where a_bill_year>=" & WrkOldYear & " and a_account=" & WrkMunCid & " and a_property_code='" & WrkMeter & "' order by a_bill_number")
        'Else
        'ds2 = myDBConnect.RunQuery("ar_history_header", "where a_bill_year>=" & WrkOldYear & " and a_account=" & WrkMunCid & " order by a_bill_number")
        'End If
        ds2 = myDBConnect.RunQuery("ar_history_header", "where a_bill_year>=" & WrkOldYear & " and a_property_code='" & WrkMeter & "' order by a_bill_number")
        For J = 0 To ds2.Tables(0).Rows.Count - 1
          WrkYear = ds2.Tables(0).Rows(J).Item("a_bill_year")
          WrkMunBillNo = ds2.Tables(0).Rows(J).Item("a_bill_number")
...
NextRec:
  ...
  Private Sub NewINV()
    ...
    MyTXINV = New TXINV(myDBConnectGEMS2.MyConn2)
    myDBConnectGEMS.OpenQry2("utcust", "where cuacct>=209082 and cuacct<=209084")
    WrkFile = "NEWINV"
    WrkType = "D"
    WrkOldYear = 2010
...
ReadNext:
  ...
  Private Sub NewHST()
    ...
    MyTXHST = New TXHST(myDBConnectGEMS.MyConn2)
    MyUTCUST = New UTCUST(myDBConnectGEMS.MyConn2)
    ds = myDBConnectGEMS.RunQuery2("NEWINV", "")
    WrkFile = "NEWHST"
    WrkOldYear = 2010
