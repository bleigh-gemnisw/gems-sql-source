...
Module PrintReport
  ...
  Public Sub PrtReport()
    ...
    If WrkFile <> String.Empty Then
      sw = New StreamWriter(MyFrmGL208B.LblFilePath.Text)
    End If
    ...
    If MyFrmGL208B.RbFileCurr.Checked Then
      GetDetail()
    Else
      GetDetailHst()
    End If
...
Done:
  ...
  Private Sub GetDetail()
    ...
    WrkQry = "PSTDT >= " & WrkDateFrom & WrkAnd & "PSTDT <= " & WrkDateTo & WrkAnd & "GLTYP<>'A'" &
 WrkAnd & "GLTYP<>'L'" & WrkAnd & "GLTYP<>'Q'"
    If WrkGltyp <> "" Then
...
ReadNext:
    ...
    If Not myLEDGERQ.IsEOF Then
      With myLEDGERQ
        Counter = Counter + 1
        If WrkFile <> String.Empty And SaveGLType <> ._GLTYP Then
...
NextRec:
  ...
  Private Sub GetBalance()
    ...
    Dim WrkRemain2 As Decimal
    Dim WrkBalance As Decimal
    Dim WrkGlTyp As String
    Dim WrkDtPost As Date
    ...
    For I = 0 To ds2.Tables(0).Rows.Count - 1
      With ds2.Tables(0).Rows(I)
        WrkDtPost = MyUtils.GetDBDate(.Item("pstdt"))
    ...
    If WrkGlTyp <> "R" Then
      ...
      WrkRemain2 = WrkBudget - WrkBalance - WrkExpend
    Else
      WrkRemain = WrkBudget + WrkExpend + WrkEncumb
      WrkRemain2 = WrkBudget + WrkExpend
    End If
    With myLEDGERQ              '@@@MAYBE
      ' 09-24-25 - Ken - Oxford wants to see 0 budget  so  tryign this per kim
      ' also this really doesnt make sense but making it this way so we can see what is being done
      ' and what was being done
      'If WrkBudget <> 0 Or WrkExpend <> 0 And WrkExpend <> 0 Or WrkRemain <> 0 Then
      If WrkBudget <> 0 Or WrkBudget = 0 Then
      ...
      " " & GetAcctDesc(._FDNBR, ._DPNBR, ._OBNBR, ._FNPGM, ._SUBFN)
        ...
        Select Case WrkGlTyp
          Case "R"
            ...
            dr.Item("amt4") = WrkRemain
            dr.Item("amt5") = 0
            dr.Item("amt6") = 0
          Case "X"
            dr.Item("amt1") = WrkOrigBudget
            dr.Item("amt2") = WrkBudget - WrkOrigBudget
            dr.Item("amt3") = WrkBudget
            dr.Item("amt4") = WrkExpend
            dr.Item("amt5") = WrkEncumb
            dr.Item("amt6") = WrkRemain
        End Select
...
NextRec:
  ...
  Private Sub GetBalanceHst()
            ...
            If .Item("pstdt") >= WrkDateFrom Then
              Select Case .Item("trtyp")
                Case "B"
                  WrkBudget = WrkBudget + (.Item("tramt") * WrkMult * -1)
                Case Else
                  If WrkDtPost.Month = MyFrmGL208B.DtPckTo.Value.Month And WrkDtPost.Year = MyFrmGL208B.DtPckTo.Value.Year Then
    ...
    With myLEDHSTQ       '@@@MAYBE
      ...
      " " & GetAcctDesc(._FDNBR, ._DPNBR, ._OBNBR, ._FNPGM, ._SUBFN)
        ...
        Select Case WrkGlTyp
          Case "R"
            dr.Item("amt1") = WrkBudget
            dr.Item("amt2") = WrkMTDExpend * -1
            dr.Item("amt3") = WrkExpend * -1
            dr.Item("amt4") = WrkPctUsed
  ...
  Public Function BuildHeadings(WrkGlType As String)
    ...
    Select Case WrkGlType
      Case "R"
        sb.Append("Amended Budget")
        sb.Append(",")
        sb.Append("Month to Date Revenue")
        sb.Append(",")
        sb.Append("Year to Date Revenue")
        sb.Append(",")
        sb.Append("YTD Pct")
        sb.Append(",")
        sb.Append("To be Collected")
        sb.Append(",")
        sb.Append("")
        sb.Append(",")
        sb.Append("")
      Case "X"
        sb.Append("Amended Budget")
        sb.Append(",")
        sb.Append("Month to Date Expenditures")
        sb.Append(",")
        sb.Append("Year to Date Expenditures")
        sb.Append(",")
        sb.Append("YTD Pct")
        sb.Append(",")
        sb.Append("Outstanding Encumbrances")
        sb.Append(",")
        sb.Append("Unencumbered Balance")
        sb.Append(",")
        sb.Append("Unexpended Balance")
    End Select
    WrkStr = sb.ToString
    sb = Nothing
    Return WrkStr
