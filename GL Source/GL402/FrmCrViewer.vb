...
End Sub
  Private Sub RunReport1()
   ...
   Dim WrkAcctFrom As String
   Dim WrkAcctTo As String

   WrkAcctFrom = ""
   WrkAcctTo = ""
   With MyFrmGL402B
     If MyUtils.CnvSng(.TxtFundFrom.Text) > 0 Then
       WrkAcctFrom = BuildAcct(MyUtils.CnvSng(.TxtFundFrom.Text), MyUtils.CnvSng(.TxtSfundFrom.Text), _
        MyUtils.CnvSng(.TxtDeptFrom.Text), MyUtils.CnvSng(.TxtObjFrom.Text), _
        MyUtils.CnvSng(.TxtFuncFrom.Text), MyUtils.CnvSng(.TxtSfuncFrom.Text))
     End If
     If MyUtils.CnvSng(.TxtFundTo.Text) > 0 Then
       WrkAcctTo = BuildAcct(MyUtils.CnvSng(.TxtFundTo.Text), MyUtils.CnvSng(.TxtSfundTo.Text), _
        MyUtils.CnvSng(.TxtDeptTo.Text), MyUtils.CnvSng(.TxtObjTo.Text), _
        MyUtils.CnvSng(.TxtFuncTo.Text), MyUtils.CnvSng(.TxtSfuncTo.Text))
     End If
   End With

   Me.Text = "Report Viewer"
   ReportPath = MyUtils.GetReportPath("PrtGL402.rpt", myTOWN._TOWNBR)
   With myreport1
    .Load(ReportPath)
    .SetDataSource(ds)
    .SetParameterValue("myreportTitle", "Transaction Journals by Batch")
    .SetParameterValue("MyUserID", MyUserID)
    .SetParameterValue("MyTownName", Trim(myTOWN._TOWN))
    .SetParameterValue("MyFromDate", MyFrmGL402B.DtPckFrom.Value)
    .SetParameterValue("MyToDate", MyFrmGL402B.DtPckTo.Value)
    .SetParameterValue("MyAcctFrom", WrkAcctFrom)
    .SetParameterValue("MyAcctTo", WrkAcctTo)
   End With
   With Crv1
     .DisplayToolbar = True
