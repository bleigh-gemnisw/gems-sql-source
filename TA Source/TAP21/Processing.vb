Module Processing
  ...
  Public Sub CopyRecords(ByVal WrkList As Integer, ByVal WrkYear As Integer)
    ...
    MyTXDCCOM.DeleteKEYcomment(WrkList, WrkLastYear)
    If ds2.Tables(0).Rows.Count > 0 Then
      For I = 0 To ds2.Tables(0).Rows.Count - 1
        With MyTXDCCOM
          .GetOneRecordP(WrkList, WrkYear, ds2.Tables(0).Rows(I).Item("seqno"))
    ...
    'MK 10/6/25 To fix Rounding issue, move Write to TXDCSUM to after TXDCDTL is written 
    SaveCode = 0
    ...
    If ds2.Tables(0).Rows.Count > 0 Then
      For I = 0 To ds2.Tables(0).Rows.Count - 1
        With MyTXDCMV
          ...
          .AddOneRecordP()
          'MK 10/6/25 Add Value to WrkNet
          WrkNet = WrkNet + ._VALUE
          'MK 10/6/25 End
        End With
      Next
    End If
    'MK 10/6/25 Write Summary code 9 record 
    If WrkNet > 0 Then
      MyTXDCCD.GetOneRecordP(WrkYear, 9, "")
      WrkValue = MyUtils.Round(WrkNet * (MyTXDCCD._ASPCT / 100), 0)
      WriteTXDCSUM(WrkList, WrkYear, 9, WrkValue, WrkNet)
    End If
    'MK 10/6/25 End
    ...
    If Not MyTXDCPP_LY.RecordNotFound Then
      ...
      With MyTXDCPP
        ._LISTNO = WrkList
        ._YEAR = WrkYear
        If MyTXDCPP_LY._STATUS = "I" Then 'Inactive keep status
          ._FILSTS = MyTXDCPP_LY._FILSTS
          ._STATUS = MyTXDCPP_LY._STATUS
        Else
          ._FILSTS = "N"
          ._STATUS = "P"
        End If
...
CopyDV:
    ...
    If ds2.Tables(0).Rows.Count > 0 Then
      For I = 0 To ds2.Tables(0).Rows.Count - 1
        With MyTXDVPI
          .GetOneRecordP(WrkList, WrkYear, ds2.Tables(0).Rows(I).Item("cat"))
          ._LISTNO = WrkList
          ._YEAR = WrkYear
...
Done:
 ...
 ByVal WrkQty As Integer)
  ...
  Public Function GetTXMSRPDEP(ByVal DeprYear As Integer) As Decimal
    ...
    Return WrkDepr
  End Function
  '  Private Function Calc2025Cost(ByVal WrkYearNo As Integer, ByVal WrkCost As Long)
  '    Dim Amount As Long

  '    Select Case WrkYearNo
  '      Case 1
  '        Amount = WrkCost * 0.95
  '      Case 2
  '        Amount = WrkCost * 0.9
