    ...
    For I = 0 To (DsTXREAL.Tables(0).Rows.Count - 1)
      With DsTXREAL.Tables(0).Rows(I)
        ...
        If .Item("twnbn") <> 0 And .Item("fcyr") = 0 Then
          Good = True
        End If
        If .Item("fccod") <> "" And .Item("fcyr") = WrkGLYear Or .Item("fccod") <> "" And WrkPrev And .Item("fcyr") = WrkYear1 Then
          Good = True
        End If
        If Good Then
          ...
          If .Item("ftax") > 0 Then
            ...
            WrkEldYear = .Item("fcyr")
          Else
            dr = dsLocal.Tables(0).NewRow
            myTXLOCAL.GetOneRecordP(.Item("list#"), WrkGLYear, "R", WrkLocPgm)
            If myTXLOCAL.RecordNotFound Then
              myTXLOCAL.GetOneRecordP(.Item("list#"), WrkGLYear - 1, "R", WrkLocPgm)
            End If
            If WrkLocPgm2 <> "" Then
              If myTXLOCAL.RecordNotFound Then
                myTXLOCAL.GetOneRecordP(.Item("list#"), WrkGLYear, "R", WrkLocPgm2)
              End If
              If myTXLOCAL.RecordNotFound Then
                myTXLOCAL.GetOneRecordP(.Item("list#"), WrkGLYear - 1, "R", WrkLocPgm2)
              End If
            End If
            WrkEldYear = myTXLOCAL._YEAR
          End If
          ...
          dr.Item("year") = WrkEldYear
          'Check for Even or Odd year
          If WrkEldYear Mod 2 = 0 Then
            dr.Item("code") = "E"
          Else
            dr.Item("code") = "O"
          End If
...
NextRec:
 ...
 ByRef WrkLocal3 As Decimal, ByRef WrkLocal4 As Decimal, ByRef WrkLocal5 As Decimal)
    Dim dstemp As DataSet = New DataSet
    Dim dstemp2 As DataSet = New DataSet
    Dim I As Integer
    ...
    WrkLocal4 = 0
    WrkLocal5 = 0
    dstemp = myTXLOCAL.GetViewbyList(WrkList, WrkGLYear, "R", 50)
    If WrkPrev And dstemp.Tables(0).Rows.Count = 0 Then
      dstemp2 = myTXLOCAL.GetViewbyList(WrkList, WrkGLYear - 1, "R", 50)
