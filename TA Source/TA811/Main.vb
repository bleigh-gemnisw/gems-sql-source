...
#End If
	...
	Public Function GetGNET(ByVal Code As String) As Boolean
		' 1. Guard clause: Check for empty input immediately before doing anything else
		If String.IsNullOrEmpty(Code) Then
			Return False
		End If

		Dim myGNET As New GNET.MyData()

		Try
			myGNET.MyDBConn = myDBConnect
			myGNET.GetOneRecordP(Code)

			' 2. Evaluate and return true only if record is found and value is "Y"
			If Not myGNET.RecordNotFound AndAlso myGNET._VALUE = "Y" Then
				Return True
			End If
		Catch ex As Exception
			Return False
		Finally
			' 3. The Finally block GUARANTEES the file/connection closes, 
			' even if the database crashes or throws an unexpected error.
			If myGNET IsNot Nothing Then
				myGNET.CloseFile()
				myGNET = Nothing
			End If
		End Try
