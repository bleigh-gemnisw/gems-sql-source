...
Dim WrkTaxExempt As String
Dim WrkFrozenFile As Boolean
	Dim WrkExmptcodes As String     'ken added 8/19/25
	'Buffered files
...
Dim WrkGroup(100) As String
	...
	With MyFrmTA215B
		...
		WrkIncomeLevel = .TxtIncomeLevel.Text
			WrkTaxExempt = .TxtTaxExempt.Text
			WrkExmptcodes = .LblExCodes.Text
			WrkFrozenFile = False
	...
	With MyCrViewer
		.Wrkds = ds
		.WrkCategory = WrkCategory
		.WrkSubdivision = WrkSubdivision
		.WrkIncomeLevel = WrkIncomeLevel
    .WrkTaxExempt = WrkTaxExempt
    .Show()
	End With
	End Sub
Friend Sub BuildDS()
...
Else
 ...
 End If
		' added 1/7/26  ken   
		If WrkExmptcodes = "* ALL Codes *" Then
			WrkExmptcodes = ""
		End If
		' end add 1/7/26 ken

		WrkSort = ""
		WrkQry = "CAT='3'" & WrkAnd & "EXMPT<>'    '"

		' ken  adeed 8/19/25 begin 
		' If user selected codes, add EXMPT WrkExmptcodes
		' --- Add EXMPT filter only if mySelexcodes has values ---
		If Not String.IsNullOrWhiteSpace(WrkExmptcodes) Then
			' Split on spaces, trim, dedupe, pad to 4 (if EXMPT is CHAR(4)), escape quotes
			Dim codesArr As String() =
				WrkExmptcodes.Split({" "c}, StringSplitOptions.RemoveEmptyEntries) _
										.Select(Function(s) s.Trim()) _
										.Where(Function(s) s.Length > 0) _
										.Distinct(StringComparer.OrdinalIgnoreCase) _
										.Select(Function(s) "'" & s.PadRight(4).Replace("'", "''") & "'") _
										.ToArray()    ' <-- materialize!

			If codesArr.Length > 0 Then
				Dim inList As String = String.Join(",", codesArr)   ' <-- join into a string
				WrkQry &= WrkAnd & "EXMPT IN (" & inList & ")"
			End If
		End If

		' ken added 8/19/25 end add 
...
For I = 0 To (DsTXREAL.Tables(0).Rows.Count - 1)
	With DsTXREAL.Tables(0).Rows(I)
		If WrkCategory <> String.Empty Then
			If WrkCategory <> Mid(.Item("exmpt"), 1, 1) Then
				Continue For
			End If
		End If
		If WrkSubdivision <> String.Empty Then
			If WrkSubdivision <> Mid(.Item("exmpt"), 2, 1) Then
				Continue For
			End If
		End If
		If WrkIncomeLevel <> String.Empty Then
			If WrkIncomeLevel <> Mid(.Item("exmpt"), 3, 1) Then
				Continue For
			End If
		End If
    If WrkTaxExempt <> String.Empty Then
      If WrkTaxExempt <> Mid(.Item("exmpt"), 4, 1) Then
        Continue For
      End If
    End If
    WrkAssCode(0) = .Item("code1")
		...
		dr.Item("units") = WrkUnits(0) + WrkUnits(1) + WrkUnits(2) + WrkUnits(3) + _
		...
		dr.Item("acres") = WrkAcres(0) + WrkAcres(1) + WrkAcres(2) + WrkAcres(3) + _
			WrkAcres(4) + WrkAcres(5) + WrkAcres(6)
		dr.Item("land") = WrkLand
		dr.Item("total") = .Item("gross")
...
Private Sub BufferGroups()
		 ...
		 Array.Clear(WrkCode, 0, 100)
		 Array.Clear(WrkGroup, 0, 100)
