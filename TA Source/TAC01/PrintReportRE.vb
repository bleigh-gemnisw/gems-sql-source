...
CloseFiles:
  ...
  Private Sub CalcPhaseInAssessment(ByVal WrkList As Integer)
    ...
    Dim MatchedVisionTotal As Integer
    Dim RemovedOrigShare As Integer
    ...
    For I = 0 To 6
      ...
      End If
    Next
    ' If an Original code no longer exists in Vision, but all
    ' remaining Vision codes match, total the matched Vision
    ' assessments so the removed Original can be redistributed.
    MatchedVisionTotal = 0

    If UnmatchedOrigTotal <> 0 AndAlso UnmatchedVisionTotal = 0 Then
      For I = 0 To 6
        If WrkCode(I) <> 0 AndAlso VisionMatched(I) Then
          MatchedVisionTotal += WrkFullAssmnt(I)
        End If
      Next
    End If
    ...
    For I = 0 To 6
      ...
      ElseIf VisionMatched(I) Then
        ...
        For J = 0 To 6
          ...
          End If
        Next
        ' A code existed in Original but no longer exists in Vision.
        ' There are no new Vision codes, so redistribute the removed
        ' Original assessment across the remaining Vision codes.
        If UnmatchedOrigTotal <> 0 AndAlso
           UnmatchedVisionTotal = 0 AndAlso
           MatchedVisionTotal <> 0 Then

          RemovedOrigShare = Math.Round(
              UnmatchedOrigTotal *
              (WrkFullAssmnt(I) / MatchedVisionTotal),
              0,
              MidpointRounding.AwayFromZero)

          WrkOrigAssmnt += RemovedOrigShare
        End If
        ' FUL = Vision - Original
