...
Module PrintReportBefore
  ...
  Dim myTXSUPPC As TXSUPPC.MyData   'added 11/12/25 kb
  Dim myTXSUPA As TXSUPA.MyData   'added 11/12/25 kb
  ...
  Public Sub PrtReportBefore()
    ...
    myTXSUPPC = New TXSUPPC.MyData(myDBConnect) '11/12/25
    myTXSUPA = New TXSUPA.MyData(myDBConnect)   '11/12/25
...
Done:
  ...
  Private Sub GetDetail()
    ...
    'Dim WrkAdjNet As Integer     11/12/25
    Dim WrkPurchNet As Integer   '11/12/25
    Dim WrkCRSaleNet As Integer   '11/12/25
    Dim WrkSaleNet As Integer     '11/12/25
    Dim WrkBTRAmt As Integer
    Dim WrkPct As Double
    ' Dim WrkSaleMonth As Integer   11/12/25
    Dim WrkMonth As Integer  '11/12/25
...
ReadNext:
    ...
    If Not myTXCOEBQ.IsEOF Then
      ...
      SaveType <> String.Empty And SaveType <> ._CTYPE Then
        ...
        WrkPct = 0
        If ._CTYPE = "M" Then
          'CalcProrateCode("M", ._CT2MC1, ._CGRS, WrkProrate, WrkAdjNet, WrkPct, WrkSaleMonth)  11/12/25 kb
          CalcProrateCode("M", ._CT2MC1, ._CGRS, WrkProrate, WrkPurchNet, WrkPct, WrkMonth)  '11/12/25  kb
        End If
        ' added 11/12/25  kb 
        If ._CTYPE = "S" Then
          CalcProrateCode("P", ._CT2MC1, ._CGRS, WrkProrate, WrkPurchNet, WrkPct, WrkMonth)
          'Check for Sale
          If Trim(._CT2MC2) <> "" Then
            CalcProrateCode("S", ._CT2MC2, ._CGRS, WrkProrate, WrkSaleNet, WrkPct, WrkMonth)
          End If
          'Check for Credit Vehicle Gross
          If Trim(._CT2MC3) <> "" Then
              CalcProrateCode("C", ._CT2MC3, ._NEWMVC, WrkProrate, WrkCRSaleNet, WrkPct, WrkMonth)
            End If
            WrkProrate = WrkPurchNet
          End If
        ' end add 11/12/25
        ...
        If ._CTYPE = "S" Then
          If MyProRateRound Then
            dr.Item("newnet") = WrkPurchNet - WrkSaleNet - WrkCRSaleNet - WrkExam
          Else
            dr.Item("newnet") = ._NTNET
          End If
        End If
        ...
        Else
          Select Case WrkType
            ...
            Case "S"
              If WrkCurrGLYear = ._CYEAR Then
                myTXSUPPC.GetOneRecordP(._LISTNo)
                If Not myTXSUPPC.RecordNotFound Then
                  With myTXSUPPC
                    FoundOrig = True
                    WrkExam = ._EXAM1 + ._EXAM2 + ._EXAM3 + ._EXAM4 + ._EXAM5
                    'MK 12/5/25 Begin
                    WrkPct = CalcSuppPct(._ASS)
                    WrkGross = CalcProRate(._VALUE, WrkPct)
                    WrkPct = CalcSuppPct(._OASS)
                    WrkCredit = CalcProRate(._OVAL, WrkPct)
                ...
                End If
              Else
                myTXSUPA.GetOneRecordP(._LISTNo, ._CYEAR)
...
NextRec:
    ...
    If dsTXCOEBL4.Tables(0).Rows.Count > 1 Then
      With dsTXCOEBL4.Tables(0).Rows(1)
        ...
        If .Item("ctype") = "M" Then
          ' CalcProrateCode("M", .Item("ct2mc1"), .Item("cgrs"), WrkProrate, WrkAdjNet, WrkPct, WrkSaleMonth)  11/12/25
          CalcProrateCode("M", .Item("ct2mc1"), .Item("cgrs"), WrkProrate, WrkPurchNet, WrkPct, WrkMonth)
        End If
        'added 11/12/25 kb
        If .Item("ctype") = "S" Then
          CalcProrateCode("P", .Item("ct2mc1"), .Item("cgrs"), WrkProrate, WrkPurchNet, WrkPct, WrkMonth)
          CalcProrateCode("S", .Item("ct2mc2"), .Item("cgrs"), WrkProrate, WrkSaleNet, WrkPct, WrkMonth)
          'Check for Credit Vehicle Gross
          If .Item("ct2mc3") <> "" Then
            CalcProrateCode("C", .Item("ct2mc3"), .Item("NEWMVC"), WrkProrate, WrkCRSaleNet, WrkPct, WrkMonth)
          End If
          WrkProrate = WrkPurchNet - WrkSaleNet - WrkCRSaleNet
        End If
        'end add 11/12/25

        If WrkProrate > 0 Then
  ...
  Public Function CalcProRate(ByVal Gross As Single, ByVal Pct As Single) As Single
    ...
    Return CalcProRate
  End Function
  Private Function CalcSuppPct(ByVal WrkCode As String) As Decimal
    Dim WrkPct As Decimal
    If WrkCode = "" Then
      Return 0
    End If

    myTXSUPCD.GetOneRecordP(WrkCode)
    If Not myTXSUPCD.RecordNotFound Then
      WrkPct = myTXSUPCD._SPCT
    End If
    Return WrkPct
  End Function
