...
#End Region
  Private Sub FrmTA8105R_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Dim WrkCreditProrate As Integer
    Dim WrkCreditAdjNet As Integer
      ...
      If Not myTXSUPPC.RecordNotFound Then
        With myTXSUPPC
          ...
          If CoeCCNo = 0 Then
            ...
            If ._OPVAL > 0 Then
              CalcProrateCode("C", Trim(._OASS), ._OVAL, WrkCreditProrate,
              WrkCreditAdjNet, WrkPurchPct, WrkPurchMonth)
            End If
            'CalcProrateCode("C", Trim(._OASS), ._OPVAL, WrkCRSaleProrate,
            ...
            'End If
            LblOrigGross.Text = FormatNumber(WrkPurchProrate + ._BTR, 0)
            LblOrigProrate.Text = FormatNumber(WrkcreditProrate, 0)
            ...
            LblOrigNet.Text = FormatNumber(MyUtils.CnvSng(LblOrigGross.Text) - MyUtils.CnvSng(LblOrigProrate.Text) -
              MyUtils.CnvSng(LblOrigAdjGross.Text) - MyUtils.CnvSng(LblOrigExam.Text), 0)
            LblOrigAssmt1.Text = WrkPurchProrate + ._BTR
            LblOrigCRAssmt1.Text = ._OVAL
    ...
    LblCRValue.Text = MyUtils.CnvSng(LblOrigCRAssmt1.Text)
    If WrkAddMode Then
...
NextCC:
  ...
  Private Sub MoveToFile()
    ...
    With myTXCOEB
      ...
      With MyGetTXMVPCT
        If MyUtils.CnvSng(TxtPurchMonth.Text) > 0 Then
          .GetTXMVPCTL1("P", MyUtils.CnvSng(TxtPurchMonth.Text))
          myTXCOEB._CT2MC1 = .Code
        End If
        If MyUtils.CnvSng(TxtSaleMonth.Text) > 0 Then
          .GetTXMVPCTL1("S", MyUtils.CnvSng(TxtSaleMonth.Text))
          myTXCOEB._CT2MC2 = .Code
        End If
        If MyUtils.CnvSng(TxtCRAssmt1.Text) > 0 Then
          .GetTXMVPCTL1("C", MyUtils.CnvSng(TxtCRSaleMonth.Text))
          myTXCOEB._CT2MC3 = .Code
        End If
      End With
      'Reason Codes
    ...
    With myTXSUPPC
      ...
      If ._RATE = 0 Then
        ...
        '._OVAL = MyUtils.CnvSng(LblCRValue.Text)
        CalcProrateMonth("P", MyUtils.CnvSng(TxtPurchMonth.Text), MyUtils.CnvSng(LblValue.Text), 0, 0, 0, WrkCode)
        ._ASS = WrkCode
      End If
    End With
  End Sub
  ...
  Public Sub GetOrigTxCOEB(ByVal WrkCCNo As Integer)
    ...
    If Not MyOrigTXCOEB.RecordNotFound Then
      With MyOrigTXCOEB
        ...
        CalcProrateCode("P", Trim(._CT2MC1), ._CGRS, WrkProrate, WrkPurchNet, WrkPct, WrkMonth)
        If MyUtils.CnvSng(TxtPurchMonth.Text) = 0 Then
          TxtPurchMonth.Text = WrkMonth
          LblPurchPct.Text = WrkPct
          LblPurchNet.Text = WrkPurchNet
        End If
        CalcProrateCode("S", Trim(._CT2MC2), ._CGRS, WrkProrate, WrkSaleNet, WrkPct, WrkMonth)
        If MyUtils.CnvSng(TxtSaleMonth.Text) = 0 Then
        ...
        If Trim(._CT2MC3) <> "" Then
          CalcProrateCode("C", Trim(._CT2MC3), ._NEWMVC, WrkProrate, WrkCRSaleNet, WrkPct, WrkMonth)
          If MyUtils.CnvSng(TxtCRSaleMonth.Text) = 0 Then
  ...
  Public Sub GetTXCOEB()
    ...
    With myTXCOEB
      ...
      CalcProrateCode("P", Trim(._CT2MC1), ._CGRS, WrkProrate, WrkAdjNet, WrkPct, WrkMonth)
      TxtPurchMonth.Text = WrkMonth
      LblPurchPct.Text = WrkPct
      LblPurchNet.Text = WrkAdjNet
      CalcProrateCode("S", Trim(._CT2MC2), ._CGRS, WrkProrate, WrkAdjNet, WrkPct, WrkMonth)
      TxtSaleMonth.Text = WrkMonth
      LblSalePct.Text = WrkPct
      LblSaleNet.Text = WrkAdjNet
      'Check for Credit Vehicle Gross
      If Trim(._CT2MC3) <> "" Then
        CalcProrateCode("C", Trim(._CT2MC3), ._NEWMVC, WrkProrate, WrkAdjNet, WrkPct, WrkMonth)
        TxtCRSaleMonth.Text = WrkMonth
        LblCRSalePct.Text = WrkPct
        LblCRSaleNet.Text = WrkAdjNet
      End If
  ...
  Public Sub CalcProrateMonth(ByVal In_Type As String, ByVal In_SaleMonth As Integer,
    ...
    'Check for valid assessment code
    If Len(WrkTxMVPCT(1)) = 1 Then
  ...
  Private Sub TxtOVMSRP_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles TxtOVMSRP.TextChanged
    ...
    WrkValue = CalcValue(WrkMSRP, WrkOvMSRP, WrkMVYear, False)
  ...
  Private Sub TxtCROVMSRP_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles TxtCROVMSRP.TextChanged
    ...
    WrkValue = CalcValue(WrkMSRP, WrkOvMSRP, WrkYear, True)
  ...
  Private Sub LnkCRSource_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles LnkCRSource.LinkClicked
    ...
    MyFrmListSource.Show()
  End Sub
  Private Function CalcValue(ByVal WrkMSRP As Integer, ByVal WrkOvMSRP As Integer, ByVal WrkMVYear As Integer,
