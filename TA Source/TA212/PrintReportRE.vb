...
Done:
  ...
  Private Sub GetDetail()
    Dim DictTXPHIN As New Dictionary(Of Integer, Long)
    Dim DictTXREALC As New Dictionary(Of Integer, Long)
    Dim dsTXPHIN As DataSet = New DataSet
    ...
    If MyPhaseIn Then
      dsTXPHIN = myTXPHIN.PosData(0, WrkYear, "list#,fulgrs")
      If dsTXPHIN.Tables.Count > 0 Then
        For Each row As DataRow In dsTXPHIN.Tables(0).Rows
          DictTXPHIN(row("LIST#")) = row("FULGRS")
        Next
      End If
      dsTXREALC = myTXREALC.PosData(0, "list#,gross")
      If dsTXREALC.Tables.Count > 0 Then
        For Each row As DataRow In dsTXREALC.Tables(0).Rows
          DictTXREALC(row("LIST#")) = row("GROSS")
        Next
      End If
    End If
    ...
    For I = 0 To (DsTXREAL.Tables(0).Rows.Count - 1)
      ...
      .Item("city"), .Item("state"), .Item("zip5"), .Item("zip4"))
        ...
        If MyPhaseIn Then
          ...
          '  End If
          'End With
          If DictTXPHIN.ContainsKey(WrkListNo) Then
            WrkFullGross = DictTXPHIN(WrkListNo) 'FULGRS
          End If
          If DictTXREALC.ContainsKey(WrkListNo) Then
            WrkPIExm = WrkFullGross - DictTXREALC(WrkListNo) 'GROSS
          End If
        End If
