...
For I = 0 To (DsTXMVD.Tables(0).Rows.Count - 1)
  ...
  With DsTXMVD.Tables(0).Rows(I)
    ...
    'added from TA205
    AddrLine = MyUtils.SetAddrLine(.Item("name"), .Item("sname"), .Item("add1"), .Item("add2"), .Item("city"), .Item("state"), .Item("zip5"), .Item("zip4"))
...
'This is code to match records in another file to pull data from additional files:
 'found = False
	myTXMVDCL1.GetOneRecordP(.Item("vinno"))
		If Not myTXMVDCL1.RecordNotFound Then
			With myTXMVDCL1
				dr.Item("lstyrval") = ._VALUE
				WrkTLYVal = WrkTLYVal + ._VALUE
			End With
		Else
			dr.Item("lstyrval") = 0
		End If
