'Suppl MV Calculation: Prorated Gross - Credit - Exemptions = Net
' Prorated Gross = Value (._VALUE) * Assessment Percentage from TXSUPCD, Code = ._ASS
' Credit = Old Value (._OVAL) * Old Assessment Percentage from TXSUPCD, Code = ._OASS
' MSRP takes ._MSRP then Vehicle Year then 70% then Prorated Percent to get new ._VALUE
' Credit MSRP takes Credit record MSRP then Vehicle Year then 70% then Credit Percent to get new ._OVAL
' If there is an override MSRP use that instead 
Module Main
...
#End If
	...
	End Function
	Public Function GetCurrentGLYear() As Integer
