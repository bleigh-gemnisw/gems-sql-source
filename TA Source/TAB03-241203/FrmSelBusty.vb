...
Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(FrmSelBusty))
...
#End Region
    ...
    Public Sub ShowGrid()
      ...
      C1DataGrdList.DataSource = mydstxbusty.Tables(0)
  ...
  Private Sub BuildDS()
    ...
    End With
    mydstxbusty.Tables.Add(myTable)
  End Sub
Private Sub RefreshDS()
  ...
  Dim Pos As Integer
    ...
    For I = 0 To ds.Tables(0).Rows.Count - 1
      myDr = mydstxbusty.Tables(0).NewRow
      Pos = InStr(MySelBusty, MyUtils.JustifyLeft(ds.Tables(0).Rows(I).Item("btcode"), 4))
      If MySelBusty = "" Or Pos > 0 Then
        myDr("select") = True
      Else
        myDr("select") = False
      End If
      myDr("code") = ds.Tables(0).Rows(I).Item("btcode")
      myDr("description") = ds.Tables(0).Rows(I).Item("btdesc")
      mydstxbusty.Tables(0).Rows.Add(myDr)
    Next
...
End Sub
  ...
  For I = 0 To (C1DataGrdList.Splits(0).Rows.Count - 1)
    If C1DataGrdList.Item(I, 0) Then
      MySelBusty = MySelBusty + MyUtils.JustifyLeft(C1DataGrdList.Item(I, 1), 4)
    Else
      WrkAllCodes = False
    End If
  Next
  ...
  Windows.Forms.Cursor.Current = Cursors.WaitCursor()
  For I = 0 To (C1DataGrdList.Splits(0).Rows.Count - 1)
    mydstxbusty.Tables(0).Rows(I).Item("Select") = True
  Next
  ...
  Windows.Forms.Cursor.Current = Cursors.WaitCursor()
  For I = 0 To (C1DataGrdList.Splits(0).Rows.Count - 1)
    mydstxbusty.Tables(0).Rows(I).Item("Select") = False
  Next
...
Private Sub FrmSelTypes_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Activated
     MyFrmTAB03.SbpScreen.Text = "SelBusty"
  End Sub
