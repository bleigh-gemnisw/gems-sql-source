...
#End Region
  ...
  Private Sub RefreshDS()
    ...
    For I = 0 To ds.Tables(0).Rows.Count - 1
      ...
      If MyBalTotal AndAlso CDec(ds.Tables(0).Rows(I).Item("wbal")) >= 0 Then
        WrkPrin = 0D
        WrkInterest = 0D
        WrkTotal = 0D
        If myTOWN._TOWNBR = 219 AndAlso {"S", "W"}.Contains(Type) Then
          CalcInterest_219SW(ListNo, Type, Year, WrkPrin, WrkInterest, WrkTotal)
        Else
          CalcInterest(ListNo, Type, Year, WrkPrin, WrkInterest, WrkTotal)
        End If
        ds.Tables(0).Rows(I).Item("wbal") = WrkTotal
      End If
    ...
    Windows.Forms.Cursor.Current = Cursors.Default
  End Sub
  Public Sub CalcInterest(ByVal InListNo As Integer, ByVal InType As String, ByVal InYear As Integer,
    ByRef OutPrin As Decimal, ByRef OutInterest As Decimal, ByRef OutDue As Decimal)
    ...
    MyCASHINT = Nothing
  End Sub
  Public Sub CalcInterest_219SW(ByVal InListNo As Integer, ByVal InType As String, ByVal InYear As Integer,
    ByRef OutPrin As Decimal, ByRef OutInterest As Decimal, ByRef OutDue As Decimal)
    ...
    End If
    OutDue = OutPrin + OutInterest + WrkLien + WrkFee
    MyCASHINT.CloseFiles()
    MyCASHINT = Nothing
  End Sub
