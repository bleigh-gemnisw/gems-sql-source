...
Module PrintReport
  ...
  Dim myBulkImport As TXA08BulkImport.MyData    'added 6/9/25
  ...
  Public Sub PrtReport()
    ...
    myBulkImport = New TXA08BulkImport.MyData(myDBConnect)
...
Done:
  ...
  Private Sub GetDetail_Bulk()
    ...
    dtStaging.Columns.Add("RecordType", GetType(String))
    dtStaging.Columns.Add("AcctID", GetType(String))
    dtStaging.Columns.Add("ListNo", GetType(Integer))
    dtStaging.Columns.Add("TaxYear", GetType(Integer))
    dtStaging.Columns.Add("TaxType", GetType(String))
    dtStaging.Columns.Add("PaidAmount", GetType(Decimal))
    dtStaging.Columns.Add("DBDatePaid", GetType(Decimal))
    dtStaging.Columns.Add("RefCode", GetType(String))
    ...
    Do While sr.Peek >= 0
      ...
      If Trim(strBuffer) = "" Then Continue Do
      ...
      Dim row As DataRow = dtStaging.NewRow()
      row("RecordType") = ""
      row("AcctID") = ""
      row("ListNo") = CInt(WrkListNo)
      row("TaxYear") = CInt(WrkYear)
      row("TaxType") = WrkType
      row("PaidAmount") = CDec(WrkPaid)
      row("DBDatePaid") = CDec(WrkDBDatePaid)
      row("RefCode") = WrkRef
      dtStaging.Rows.Add(row)
    Loop
    sr.Close()

    If Not myBulkImport.UploadToStaging(dtStaging) Then
      MsgBox("UploadToStaging failed")
      myFrmProgress.Close()
      Exit Sub
    End If

    Dim dtResult As DataTable = myBulkImport.GetMatchedTXINV()
    If dtResult.Rows.Count = 0 Then
      MsgBox("No matched records returned")
      myFrmProgress.Close()
      Exit Sub
    End If
    ...
    TotLien = 0
    TotBond = 0
    WrkBatchNo = myBCHHDR.AutoGenKey("PTC")
    myBCHHDR.GetOneRecordP("PTC", WrkBatchNo)

    For Each dr As DataRow In dtResult.Rows
      WrkListNo = CInt(dr("ListNo"))
      WrkYear = CInt(dr("TaxYear"))
      WrkType = dr("TaxType").ToString()
      WrkPaid = CDec(dr("PaidAmount"))
      WrkDBDatePaid = CDec(dr("DBDatePaid"))
      WrkRef = dr("RefCode").ToString()
      WrkReceiptDate = Today
      WrkInterestDate = Today
      ...
      With myTCRBCH
        ...
        ._NAME = If(dr.Table.Columns.Contains("NAME"), Trim(dr("NAME").ToString()), "")
        ._DIST = If(dr.Table.Columns.Contains("DIST"), CInt(dr("DIST")), 0)

        CalcInterest(WrkListNo, WrkType, WrkYear, MyUtils.SetDBDate(WrkInterestDate),
                   WrkInterest, WrkInterestPaid, WrkFeeDue,
        ...
        WrkPrevLienPaid = 0
        WrkPrevFeePaid = 0
        If CheckForDup(WrkBatchNo, WrkListNo, WrkType, WrkYear) Then
          PrevTrans(WrkBatchNo, WrkListNo, WrkType, WrkYear, WrkTrnbr,
                  WrkPrevTaxPaid, WrkPrevIntPaid, WrkPrevLienPaid, WrkPrevFeePaid)
          WrkDue -= WrkPrevTaxPaid
          WrkInterest -= WrkPrevIntPaid
          WrkLien -= WrkPrevLienPaid
        End If
        ...
        If WrkPaid >= (WrkTax + WrkLien) AndAlso WrkLien > 0 Then
          WrkPaid -= WrkLien
        Else
          WrkLien = 0
        End If
        ...
        WrkFee(0) = CDec(dr("FED1"))
        WrkFee(1) = CDec(dr("FED2"))
        WrkFee(2) = CDec(dr("FED3"))
        WrkFee(3) = CDec(dr("FED4"))
        WrkFee(4) = CDec(dr("FED5"))
        WrkFeecd(0) = dr("FEC1").ToString()
        WrkFeecd(1) = dr("FEC2").ToString()
        WrkFeecd(2) = dr("FEC3").ToString()
        WrkFeecd(3) = dr("FEC4").ToString()
        WrkFeecd(4) = dr("FEC5").ToString()
        WrkFee(5) = 0
        WrkFeecd(5) = ""

        If dr("MVFLAG").ToString() = "Y" OrElse (dr("MVFLAG").ToString() = "M" AndAlso WrkFeeDue > 0) Then
          WrkFee(5) = MyMVFee
          WrkFeecd(5) = "MV"
          WrkFeeDue -= WrkFee(5)
        End If
        ...
        If WrkPrevFeePaid > 0 Then
          PrevPaidFee(WrkBatchNo, WrkListNo, WrkType, WrkYear, WrkTrnbr, WrkFee, WrkFeecd)
        End If

        For I = 0 To 4
          If WrkFee(I) > 0 AndAlso WrkFeeDue > 0 Then
        ...
        ._REF = WrkRef
        ._COMM = "WEB PAYMENT"
        ._PMETH = If(WrkCheck, "2", "3")
        ._SRC = 5

        '.AddOneRecordP()  ' changed to speed up record adding
        .AddOneRecordFast()
      ...
      WrkPct = (TotCount / dtResult.Rows.Count) * 100
