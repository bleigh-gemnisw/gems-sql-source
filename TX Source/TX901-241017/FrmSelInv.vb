...
End Sub
  ...
  Private Sub FrmSelInvF_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    If MyQrySelect <> WrkQrySelect Then
       ...
       End If
       If RbName.Checked Then
        ds = mytxinvl8.GetViewbyName(TxtPos.Text, WrkListNo, WrkYear, WrkType, WrkQrySelect, MyGridMax, MyBlocking)
       End If
       If RbSName.Checked Then
        ds = mytxinvls.GetViewbySName(TxtPos.Text, WrkListNo, WrkYear, WrkType, WrkQrySelect, MyGridMax, MyBlocking)
       End If
    If RbLoc.Checked Then
      ...
      Do While Len(WrkPosNo) < 7 'Left pad with blanks
        WrkPosNo = " " & WrkPosNo
      Loop
      ds = mytxinvla.GetViewbyLoc(TxtPos.Text, WrkPosNo, WrkListNo, WrkYear, WrkType, WrkQrySelect, MyGridMax, MyBlocking)
    End If
    If RbReg.Checked Then
        ds = mytxinvlc.GetViewbyRegNo(TxtPos.Text, WrkQrySelect, MyGridMax, MyBlocking)
       End If
