...
Public Class FrmTXA094
  ...
  Const WrkMaxRecs As Integer = 25
  ...
  Private InitialLoad As Boolean = True  ' added 6/2/25   to bypass calling formatgrid so many times to start
...
#End Region
  ...
  Private Sub RefreshDS()
    ...
    For I = 0 To ds.Tables(0).Rows.Count - 1
      ...
      If dstxbatch.Tables(0).Rows.Count > 0 Then
        ds.Tables(0).Rows(I).Item("wupost") = "*"
      End If
      ...
      If MyBalTotal And ds.Tables(0).Rows(I).Item("wbal") > 0 Then
        ...
        WrkInterest = 0D
        WrkTotal = 0D
        If myTOWN._TOWNBR = 219 AndAlso {"S", "W"}.Contains(Type) Then
          CalcInterest_219SW(ListNo, Type, Year, MyInterestDate, 0, WrkInterest, WrkFee, WrkLien, WrkBond, WrkTotal)
        Else
          CalcInterest(ListNo, Type, Year, MyInterestDate, 0, WrkInterest, WrkFee, WrkLien, WrkBond, WrkTotal)
        End If
        WrkBaldue = ds.Tables(0).Rows(I).Item("wbal") + WrkInterest + WrkFee + WrkLien + WrkBond
        ds.Tables(0).Rows(I).Item("wbal") = Format(WrkBaldue, "Fixed")
      End If
      ...
      If C1DataGrdList.Item(I, 0) = 1 And C1DataGrdList.Item(I, 10) <> "I" Then
        ...
        Select Case CboSort.SelectedItem.ToString
          ...
          Case Else
            ...
            myDr("Balance") = C1DataGrdList.Item(I, 8)
        End Select
        If myTOWN._TOWNBR = 219 AndAlso {"S", "W"}.Contains(myDr("Type")) Then
          CalcInterest_219SW(myDr("ListNo"), myDr("Type"), myDr("Year"), MyInterestDate, WrkPrin, WrkInterest, WrkFee, WrkLien, WrkBond, WrkTotal)
        Else
          CalcInterest(myDr("ListNo"), myDr("Type"), myDr("Year"), MyInterestDate, WrkPrin, WrkInterest, WrkFee, WrkLien, WrkBond, WrkTotal)
        End If
        myDr("Tax") = Format(WrkPrin, "standard")
        myDr("Interest") = Format(WrkInterest, "standard")
        myDr("Fee") = Format(WrkFee, "standard")
        myDr("Lien") = Format(WrkLien, "standard")
        myDr("Bond") = Format(WrkBond, "standard")
        myDr("Total") = Format(WrkTotal, "standard")
  ...
  Sub GetListNoRecords()
    ...
    For I = 0 To ds2.Tables(0).Rows.Count - 1
      WrkType = ds2.Tables(0).Rows(I).Item("type")
      WrkYear = ds2.Tables(0).Rows(I).Item("year")
      If TxtType.Text <> "" And TxtType.Text <> wrktype Then
        Continue For
      End If
      If TxtYear.Text <> "" And MyUtils.CnvSng(TxtYear.Text) <> WrkYear Then
        Continue For
      End If
      ...
      myDr("Year") = WrkYear
      With myTXINV
        .GetOneRecordP(Val(TxtList.Text), WrkYear, WrkType)
        If Not .RecordNotFound Then
          ...
          If MyBalTotal And myDr("wbal") >= 0 Then
            If myTOWN._TOWNBR = 219 AndAlso {"S", "W"}.Contains(WrkType) Then
              CalcInterest_219SW(Val(TxtList.Text), WrkType, WrkYear, MyInterestDate, 0, WrkInterest, WrkFee, WrkLien, WrkBond, WrkTotal)
            Else
              CalcInterest(Val(TxtList.Text), WrkType, WrkYear, MyInterestDate, 0, WrkInterest, WrkFee, WrkLien, WrkBond, WrkTotal)
            End If
            WrkBaldue = myDr("wbal") + WrkInterest + WrkFee + WrkLien + WrkBond
            'myDr("wbal") = Format(WrkTotal, "standard")
            myDr("wbal") = Format(WrkBaldue, "standard")
          End If
