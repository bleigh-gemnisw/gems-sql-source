...
Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(FrmSelReason))
...
#End Region
		...
		Private Sub FrmSelReason_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
			myTxsresn = New TXSRESN.mydata(MyDBConnect)
			MyFrmTX904.TBarProcess.Enabled = False
			BuildDS()
			RefreshDS()
			Call FormatGrid()
    ...
    Public Sub ShowGrid()
      ...
      C1DataGrdList.DataSource = mydstxsresn.Tables(0)
  ...
  Private Sub BuildDS()
    ...
    End With
    mydstxsresn.Tables.Add(myTable)
  End Sub
Private Sub RefreshDS()
  ...
  Dim Pos As Integer
    ...
    For I = 0 To ds.Tables(0).Rows.Count - 1
      myDr = mydstxsresn.Tables(0).NewRow
      Pos = InStr(Mysresn, ds.Tables(0).Rows(I).Item("sresn"))
      If Pos > 0 Then
        myDr("select") = True
      Else
        myDr("select") = False
      End If
      myDr("Code") = ds.Tables(0).Rows(I).Item("sresn")
      myDr("description") = ds.Tables(0).Rows(I).Item("srdesc")
      mydstxsresn.Tables(0).Rows.Add(myDr)
    Next
...
End Sub
  ...
  For I = 0 To (C1DataGrdList.Splits(0).Rows.Count - 1)
    If C1DataGrdList.Item(I, 0) Then
      Mysresn = Mysresn + C1DataGrdList.Item(I, 1)
    Else
      WrkAllTypes = False
    End If
  Next
  ...
  Windows.Forms.Cursor.Current = Cursors.WaitCursor()
  For I = 0 To (C1DataGrdList.Splits(0).Rows.Count - 1)
    mydstxsresn.Tables(0).Rows(I).Item("Select") = True
  Next
  ...
  Windows.Forms.Cursor.Current = Cursors.WaitCursor()
  For I = 0 To (C1DataGrdList.Splits(0).Rows.Count - 1)
    mydstxsresn.Tables(0).Rows(I).Item("Select") = False
  Next
  ...
  Windows.Forms.Cursor.Current = Cursors.Default
  End Sub
Private Sub FrmSelReason_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Activated
     MyFrmTX904.SbpScreen.Text = "SelReason"
  End Sub
  ...
  Private Sub FrmSelReason_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
...
Private Sub FrmSelReason_Closed(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Closed
  MyFrmTX904.TBarProcess.Enabled = True
  MyFrmTX904B.txtsresn.Text = MySresn
End Sub
End Class
