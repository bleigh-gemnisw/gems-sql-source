...
NextLine:
    ...
    If WrkPeople And Not WrkBusiness And Trim(sArray(0)) = String.Empty Then
      GoTo NextRec
    End If
    If WrkBusiness And Not WrkPeople And Trim(sArray(3)) = String.Empty Then
      GoTo NextRec
    End If
    WrkClass = GetTXVCLSCode(sArray(6))
    WrkRegno = sArray(5)
    If SaveRegno = WrkRegno Then GoTo NextRec
    SaveRegno = WrkRegno
    WrkTown = MyUtils.CnvSng(sArray(4))
    ...
    If sArray(3) <> String.Empty Then
      WrkNName = sArray(3)
    Else
      WrkNName = sArray(0)
      If sArray(1) <> String.Empty Then
        WrkNName = WrkNName & " " & sArray(1)
      End If
      If sArray(2) <> String.Empty Then
        WrkNName = WrkNName & " " & sArray(2)
      End If
    End If
    WrkNAddr = sArray(12)
    WrkNAddr2 = sArray(13)
    WrkNCity = sArray(14)
    WrkNState = sArray(15)
    WrkNZip5 = Mid(sArray(16), 1, 5)
    If Len(sArray(16)) > 5 Then
      WrkNZip4 = Mid(sArray(16), 6, 4)
    Else
      WrkNZip4 = ""
    End If
...
NextLine:
    ...
    If WrkPeople And Not WrkBusiness And Trim(sArray(3)) = String.Empty Then
      GoTo NextRec
    End If
    If WrkBusiness And Not WrkPeople And Trim(sArray(6)) = String.Empty Then
      GoTo NextRec
    End If
    WrkClass = GetTXVCLSCode(sArray(2))
    WrkRegno = sArray(1)
    If SaveRegno = WrkRegno Then GoTo NextRec
    SaveRegno = WrkRegno
    WrkTown = MyUtils.CnvSng(sArray(0))
    WrkOName = String.Empty
    myTXMVDCL2.GetOneRecordP(WrkClass, WrkRegno)
    If Not myTXMVDCL2.RecordNotFound Then
      With myTXMVDCL2
    ...
    Else
      WrkNName = sArray(3)
      If sArray(4) <> String.Empty Then
        WrkNName = WrkNName & " " & sArray(4)
      End If
