  ...
  & WrkAnd & "YEAR <= " & WrkToGLYear
    ...
    For I = 0 To (DsTXHSTQ.Tables(0).Rows.Count - 1)
      With DsTXHSTQ.Tables(0).Rows(I)
        ' check to see if already released if so skip it
        MyTXHST.CheckifLienReleased(.Item("list#"), .Item("type"), .Item("year"))
        If Not MyTXHST.RecordNotFound Then
          GoTo NextRec
        End If
        ...
        DsTXHST = MyTXHSTL4.GetViewbyList(.Item("list#"), .Item("year"), .Item("type"), 0, 999999)
        ...
        If WrkPost Then
          WriteTXHST(.Item("list#"), .Item("year"), .Item("type"))
        End If
      End With
...
CloseFiles:
  ...
  Private Sub WriteTXHST(ByVal ListNo As Integer, ByVal Year As Integer, ByVal Type As String)
    ...
    With MyTXHST
      ...
      ._BATCHN = 0
      ._BATCHA = ""
      ._PDATE = MyUtils.SetDBDate(Date.Now.Date)
