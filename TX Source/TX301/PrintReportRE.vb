...
Module PrintReportRE
  ...
  Dim WrkTotDesc(7) As String
  Dim WrkTotCount(7) As Integer
  Dim WrkTotGross(7) As Long
  Dim WrkTotExemption(7) As Integer
  Dim WrkTotNet(7) As Long
  Dim WrkTotTax(7) As Decimal
  Dim WrkTot1st(7) As Decimal
  Dim WrkTot2nd(7) As Decimal
  ...
  Private Sub GetDetail()
    ...
    For I = 0 To (DsTXREALC.Tables(0).Rows.Count - 1)
      With DsTXREALC.Tables(0).Rows(I)
        ...
        'MK 7/10/25 Begin 
        If WrkDeferT > 0 Then
          WrkTot1st(7) = WrkTot1st(7) + WrkDefer1st
          WrkTot2nd(7) = WrkTot2nd(7) + WrkDefer2nd
          WrkTotTax(7) = WrkTotTax(7) + WrkDeferT
        End If
        'MK 7/10/25 End
...
NextRec:
    ...
    For K = 0 To WrkMaxK
      ...
      Select Case K
        ...
        Case 6
          ...
          'MK 7/10/25 Begin
        Case 7
          drTot.Item("description") = "Deferred"
          'MK 7/10/25 End
      End Select
  ...
  Private Function DownloadCSV(ByVal I As Integer) As String
    ...
    With DsTXREALC.Tables(0).Rows(I)
      ...
      '''''''''''ADDED Lou 2026-01-09 - start

      '''''''''''Append a new CSV column for the QR URL. Used specs from Bob embedded in the code below,See "Spec:" keyword for them

      '''''''''''SPEC: wtcid Is a rcdid auto increment, (If its Not in my code on SU you dont need it)

      '''''''''''SPEC: sessionid , (If its Not in my code on SU you dont need it)

      '''''''''''SPEC: txlist Is a composite Of the type year, TypeCode And List regardless of the type
      '''''''''''''my Comments
      '''''''''''''appears to be Like the qrAcctId_SU in the SU code, Mid(WrkGLYear.ToString("0000"), 3, 2) & WrkType & WrkList.ToString("000000")
      ''''''''''Dim txList As String = Mid(WrkGLYear.ToString("0000"), 3, 2) & WrkType & WrkList.ToString("000000")   ' added to call

      '''''''''''SPEC: productidds come from wherever im getting them now On SU, If im Not getting them they are being loaded on the backend by the php program
      ''''''''''Dim productId_QR As String = WrkType & WrkGLYear.ToString("0000")

      '''''''''''SPEC: amt Is field called bald i think (balance due)
      '''''''''''''my Comments
      '''''''''''''I believe its the WrkTaxTotal based on that this field is used to set the BALD

      '''''''''''SPEC: name Is always the name
      ''''''''''Dim ownerName_QR As String = CStr(.Item("name")) & " " & CStr(.Item("sname"))

      '''''''''''SPEC: Type Is a desc of the code that comes from the txtype table i believe (prob already have a lookup in the SU code)
      '''''''''''''my Comments
      '''''''''''''WrkType = "R"  'was hardcoded in the above existing code as like in the SU code. 

      '''''''''''SPEC: desc - Is the car desc on cars an d the property location i believe on real estate And desc on personal property
      '''''''''''''my Comments
      '''''''''''''available in teh code above are
      '''''''''''''sb.Append(.Item("loc#"))   --- Used this
      '''''''''''''sb.Append(.Item("loc"))    --- Used this
      '''''''''''''sb.Append(.Item("bkcd"))
      '''''''''''''sb.Append(.Item("map"))
      '''''''''''''sb.Append(.Item("vol"))
      '''''''''''''sb.Append(.Item("pge"))
      ''''''''''Dim desc_QR As String = CStr(.Item("loc#")) & " " & CStr(.Item("loc"))

      '''''''''''SPEC: txname shouold be the name
      '''''''''''''my Comments
      '''''''''''''I beleive this was already addressed above?
      ...
      ''''''''''  price:=WrkTaxTotal,
      ''''''''''  txName:=ownerName_QR,
      ''''''''''  txType:=WrkType,            'R was hard coded in the original code above
      ''''''''''  txDesc:=desc_QR
      '''''''''')
