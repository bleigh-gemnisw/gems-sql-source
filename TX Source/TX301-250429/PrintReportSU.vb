...
Module PrintReportSU
  ...
  Dim WrkTotExemption(5) As Long
  Dim WrkTotNet(5) As Long
  Dim WrkTotProrate(5) As Decimal
  Dim WrkTotCredit(5) As Decimal
  ...
  Private Sub GetDetail()
    ...
    For I = 0 To (DsTXSUPP.Tables(0).Rows.Count - 1)
      ...
      With DsTXSUPP.Tables(0).Rows(I)
        WrkCC = False
        'Suppl don't have before bill C/C's
        '    If .Item("ccno") > 0 Then
        '      WrkCC = True
        '    End If
        WrkList = .Item("list#")
      ...
      .Item("city"), .Item("state"), .Item("zip5"), .Item("zip4"))
        ...
        WrkVIN = True
        'Wilton: No VIN in Credit Vehicle Description
        If myTOWN._TOWNBR = 161 Then WrkVIN = False
  ...
  Private Sub WriteMSInvoice(ByVal I As Integer)
    ...
    With DsTXSUPP.Tables(0).Rows(I)
      ...
      Else
        myTXINV._DIST = 0
      End If
      ds2 = myTXVEHL1.GetViewRegNo(.Item("regno"), 1)
      If ds2.Tables(0).Rows.Count > 0 Then
        If Trim(ds2.Tables(0).Rows(0).Item("regno")) = Trim(.Item("regno")) Then
          myTXVEH.GetOneRecordP(ds2.Tables(0).Rows(0).Item("vehid"))
          Pos = InStr(Trim(myTXVEH._DADD1), " ", CompareMethod.Text)
          If Pos > 0 Then
            myTXINV._LOC = Mid(Trim(myTXVEH._DADD1), Pos + 1, 25)
            myTXINV._LOCNo = MyUtils.JustifyRight(Mid(Trim(myTXVEH._DADD1), 1, Pos - 1), 7)
          Else
