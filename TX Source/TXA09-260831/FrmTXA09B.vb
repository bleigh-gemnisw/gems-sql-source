...
#End Region
  ...
  Public Sub LoadInterest()
    ...
    Dim WrkFeePaid As Decimal
    Dim WrkDebug As String

    WrkListno = MyUtils.CnvSng(LblList.Text)
    WrkType = LblType.Text
    WrkYear = MyUtils.CnvSng(LblYear.Text)
    WrkDebug = ""
    If myTOWN._TOWNBR = 219 And {"S", "W"}.Contains(WrkType) Then
      CalcInterest_219SW(WrkListno, WrkType, WrkYear, DtPckInt.Value, WrkPrin, WrkInterest, WrkFee, WrkLien, WrkBond, WrkTotal, WrkCAFee, WrkIntPaid, WrkFeePaid, WrkDebug)
    Else
      CalcInterest(WrkListno, WrkType, WrkYear, DtPckInt.Value, WrkPrin, WrkInterest, WrkFee, WrkLien, WrkBond, WrkTotal, WrkCAFee, WrkIntPaid, WrkFeePaid, WrkDebug)
    End If
    LblInterest.Text = Format(WrkInterest, "standard")
    LblLien.Text = Format(WrkLien, "standard")
    LblFee.Text = Format(WrkFee, "standard")
    LblCAFee.Text = Format(WrkCAFee, "standard")
    LblBond.Text = Format(WrkBond, "standard")
    LblTax.Text = Format(WrkPrin, "standard")
    LblDue.Text = Format(WrkTotal, "standard")
    LblIntPaid.Text = Format(wrkIntPaid, "standard")
    If LblBondPaidHdr.Text = "Fee Paid" Then
      LblBondPaid.Text = Format(WrkFeePaid, "standard")
    End If
    WrkCashIntDebug = WrkDebug
