...
Module PrintReportRESwr
  ...
  Dim dsSwr As DataSet = New DataSet
  Dim dsEscrow As DataSet = New DataSet
  Dim dsEscrowSwr As DataSet = New DataSet
  ...
  Dim WrkTotDesc(8) As String
  Dim WrkTotCount(8) As Integer
  Dim WrkTotGross(8) As Long
  Dim WrkTotExemption(8) As Integer
  Dim WrkTotNet(8) As Long
  Dim WrkTotTax(8) As Decimal
  Dim WrkTot1st(8) As Decimal
  Dim WrkTot2nd(8) As Decimal
  Dim WrkList As Integer
  Dim WrkType As String
  Dim WrkTypeUB As String
  ...
  Public Sub PrtReportRESwr()
    ...
    'Clear Totals
    Array.Clear(WrkTotCount, 0, 8)
    Array.Clear(WrkTotGross, 0, 8)
    Array.Clear(WrkTotExemption, 0, 8)
    Array.Clear(WrkTotNet, 0, 8)
    Array.Clear(WrkTotTax, 0, 8)
    Array.Clear(WrkTot1st, 0, 8)
    Array.Clear(WrkTot2nd, 0, 8)
    ...
    If ds.Tables.Count = 0 Then
      ...
      dsSwr = ds.Clone
      dsEscrow = ds.Clone
      dsEscrowSwr = ds.Clone
      BuildDSBillComb(dsBill)
      BuildDSTot(dsTot)
    Else
      ds.Clear()
      dsSwr.Clear()
        ...
        Else
          ...
          For J = 0 To 6
            ...
            If Trim(WrkExcd(J)) <> "" And WrkExam(J) = 0 Then
              ...
              WrkExemption = WrkExemption + WrkExFixedAmt(K)
            Else
              WrkExemption = WrkExemption + WrkExam(J)
            End If
          Next
        End If
        WrkNet = WrkGross - WrkExemption
...
Sewer:
        ...
        WrkSewerUse1st = 0
        WrkSewerUse2nd = 0
        MyUTCUST.GetOneRecordP(WrkList)
        If Not MyUTCUST.RecordNotFound Then
          MyUTCUSTRT.GetOneRecordP(WrkList, WrkTypeUB)
          With MyUBCalcBillU
            If MyUTCUST._CUUNIT > 0 Then
              .In_Units = MyUTCUST._CUUNIT
              .In_RateType = WrkTypeUB
              .In_RateCode = MyUTCUSTRT._CRCODE
              .CalcUsage()
              WrkSewerUse = .Out_Bill
            End If
          End With
          If WrkSewerUse > 0 Then
            With myTPaymnt
              .In_ListNo = WrkList
              .In_Type = WrkType 'WrkTypeUB
              .In_Year = WrkGLYear
              .In_Dst = WrkDist
              .In_Phs = WrkPhase
              .In_TaxT = WrkSewerUse
              .CalcPaySplit()
              WrkSewerUse = .Out_TaxT
              WrkSewerUse1st = .Out_Tax1
              WrkSewerUse2nd = .Out_Tax2
            End With
          End If
        End If

        If WrkSewerUse > 0 Then
          WrkTotCount(7) = WrkTotCount(7) + 1
          WrkTotTax(7) = WrkTotTax(7) + WrkSewerUse
          WrkTot1st(7) = WrkTot1st(7) + WrkSewerUse1st
          WrkTot2nd(7) = WrkTot2nd(7) + WrkSewerUse2nd
        End If
      ...
      .Item("city"), .Item("state"), .Item("zip5"), .Item("zip4"))
        ...
        If WrkSewerUse > 0 Then
          'Create Report (Sewer)
          dr = dsSwr.Tables(0).NewRow
          dr.Item("listno") = WrkList
          dr.Item("addr1") = AddrLine(0)
          dr.Item("gross") = 0
          dr.Item("exemption") = 0
          dr.Item("net") = 0
          dr.Item("taxtot") = WrkSewerUse
          dr.Item("group") = ""
          dsSwr.Tables(0).Rows.Add(dr)
        End If

        'Filter - Omit Zero Bills 
        If WrkTaxTotal = 0 And WrkSewerUse = 0 Then
          GoTo WriteInvoice
        End If
        ...
        If MyFrmTX301B.RbPrtBill.Checked Or MyFrmTX301B.LblFilePath.Text <> String.Empty Then
          Select Case myTXFMBILL._SCAN
            Case "S"
              WrkScanLine = BuildScanLineSewer(WrkList, WrkGLYear, WrkType, WrkTaxTotal + WrkSewerUse,
         WrkTax1st, WrkTax2nd, WrkSewerUse1st, WrkSewerUse2nd, .Item("btc"))
            Case "W"
              WrkScanLine = BuildScanLineWebster(WrkList, WrkGLYear, WrkType, WrkTaxTotal + WrkSewerUse,
         WrkTax1st, WrkTax2nd, WrkSewerUse1st, WrkSewerUse2nd, .Item("btc"))
            Case Else
        ...
        If WrkNonEscrow Then
          If Trim(WrkBankCode) <> String.Empty Or Trim(.Item("bksv")) <> String.Empty Then
            ...
            'Create Escrow List (Sewer)
            If WrkSewerUse > 0 Then
              dr = dsEscrowSwr.Tables(0).NewRow
              dr.Item("listno") = WrkList
              dr.Item("addr1") = AddrLine(0)
              dr.Item("gross") = 0
              dr.Item("exemption") = 0
              ...
              Else
                dr.Item("group") = "SV:" & .Item("bksv")
              End If
              dsEscrowSwr.Tables(0).Rows.Add(dr)
              WrkIsEscrow = True
            End If
            GoTo WriteExport
          End If
     ...
     .Item("city"), .Item("state"), .Item("zip5"), .Item("zip4"))
        ...
        drBill.Item("othtot") = WrkSewerUse
        drBill.Item("oth1st") = WrkSewerUse1st
        drBill.Item("oth2nd") = WrkSewerUse2nd
        drBill.Item("billtot") = WrkTaxTotal + WrkSewerUse
        drBill.Item("bill1st") = WrkTax1st + WrkSewerUse1st
        drBill.Item("bill2nd") = WrkTax2nd + WrkSewerUse2nd
...
NextRec:
    ...
    For K = 0 To 7
      ...
      Select Case K
        ...
        Case 7
          drTot.Item("description") = "Sewer Use Fee"
      End Select
  ...
  Private Function DownloadREBill(ByVal I As Integer) As String
    ...
    With DsTXREALC.Tables(0).Rows(I)
      ...
      Else
        WrkInteger = MyUTCUST._CUUNIT * 100
  ...
  Private Sub WriteREInvoice(ByVal I As Integer)
    With DsTXREALC.Tables(0).Rows(I)
      ...
      myTXINV._TX3RD = WrkTax3rd
      myTXINV._TX4TH = WrkTax4th
    ...
    End If
  End Sub
  Private Sub WriteUBInvoice(ByVal I As Integer)

    If WrkSewerUse = 0 Then Exit Sub

    With DsTXREALC.Tables(0).Rows(I)
      myTXINVUB.GetOneRecordP(WrkList, WrkGLYear, WrkTypeUB)
      If .Item("btc") = "BT" Then
        myTXINVUB._ICODE = "B"
      Else
        myTXINVUB._ICODE = ""
      End If
      myTXINVUB._LISTNo = WrkList
      myTXINVUB._YEAR = WrkGLYear
      myTXINVUB._TYPE = WrkTypeUB
      myTXINVUB._NAME = .Item("name")
      myTXINVUB._SNAME = .Item("sname")
      myTXINVUB._ADD1 = .Item("add1")
      myTXINVUB._ADD2 = .Item("add2")
      myTXINVUB._CITY = .Item("city")
      myTXINVUB._STATE = .Item("state")
      myTXINVUB._ZIP5 = Format(.Item("zip5"), "00000")
      myTXINVUB._ZIP4 = Format(.Item("zip4"), "0000")
      If WrkUpdateDist Then
        myTXINVUB._DIST = .Item("dist")
      Else
        myTXINVUB._DIST = 0
      End If
      myTXINVUB._PDST = .Item("pdst")
      myTXINVUB._TAXT = WrkSewerUse
      myTXINVUB._TAX1 = WrkSewerUse1st
      myTXINVUB._TAX2 = WrkSewerUse2nd
      myTXINVUB._BALD = WrkSewerUse
      myTXINVUB._LOCNo = MyUtils.JustifyRight(.Item("loc#"), 7)
      myTXINVUB._LOC = .Item("loc")
      myTXINVUB._BKCD = .Item("bkcd")
      myTXINVUB._MAP = .Item("map")
      myTXINVUB._VOL = .Item("vol")
      myTXINVUB._IPAGE = .Item("pge")
      myTXINVUB._LETT = .Item("lett")
    End With
    myTXINVUB.AddOneRecordP()
