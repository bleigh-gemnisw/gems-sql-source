  ...
  Private Sub GetDetail()
    ...
    For I = 0 To (dsFile.Tables(0).Rows.Count - 1)
      With dsFile.Tables(0).Rows(I)
        ...
        myTXINVTo.GetOneRecordVIN(.Item("imvid#"), WrkToYear, WrkToType)
        If myTXINVTo.RecordNotFound Then GoTo NextRec
        ' using the found record list# not the from list#
        WriteTXINV(myTXINVTo._LISTNo, WrkToYear, WrkToType)
        If GoodStatus Then
          dr = ds.Tables(0).NewRow
          dr.Item("listno") = myTXINVTo._LISTNo   'to list#
          dr.Item("year") = WrkToYear
          dr.Item("type") = WrkToType
  ...
  Private Sub WriteTXINV(ByVal WrkListNo As Integer, ByVal WrkYear As Integer, ByVal WrkType As String)
    With myTXINVTo
      ...
      If GoodStatus Then
        ...
        If WrkPost Then
          .UpdateOneRecordP()
          WriteHistorymv(WrkListNo, WrkYear, WrkType)
        End If
      End If
    End With
  End Sub
  Public Sub WriteHistorymv(ByVal WrkListNo As Integer, ByVal WrkYear As Integer, ByVal WrkType As String)
