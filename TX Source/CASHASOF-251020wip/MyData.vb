...
#Region "Variables"
  ...
  Dim myHistRecs As New HistRecs
...
#Region "Subroutines"
  ...
  Public Sub CalcAsof()
   ...
   InvDist <> myTXPROF._DIST Then
    ...
    WrkDate = 0
    WrkTotRec = 0
    myHistRecs = ProcessHistory(In_ListNo, In_Year, In_Type, WrkDate, In_AsofDate, ProfPrPerd, ProfTxGd, WrkAdj)
    With myHistRecs
      WrkAdjFnd = .AdjFnd
      WrkAdjAmt = .AdjAmt
      WrkIntDate = .IntDate
...
NextPerd:
    ...
    If WrkAdjFnd Then
      ...
      If WrkIntDate(X) > WrkIntDate(WrkInstallNo) Then
        If WrkAdjAmt = 0 Then
          ...
          WrkPrinPd(X) = False
        Else
          WrkIntDate(X) = myHistRecs.OutIntDate
        End If
      End If
    End If
    ...
    If myHistRecs.OutIntDate <> ChkDate Then
      ...
      If WrkIntDate(X) <= ProfTxGd(X) Then
        WrkIntDate(X) = ProfTxDt(X)
        sbDebug.Append(vbCrLf & "Hist Date/Pre-Grace(" & X & "):" & myHistRecs.OutIntDate)
      End If
    End If
...
#End Region
  ...
  Private Function Round(ByVal Number As Decimal, ByVal Decimals As Integer) As Decimal
    ...
    Return WrkNo
  End Function
  Public Function ProcessHistory(ByVal In_ListNo As Integer,
                               ByVal In_Year As Integer,
                               ByVal In_Type As String,
                               ByVal WrkDate As Integer,
                               ByVal In_AsofDate As Date,
                               ByVal ProfPrPerd As Integer,
                               ByVal ProfTxGd() As Date,
                               ByVal WrkAdj() As Decimal) As HistRecs

    Dim result As New HistRecs
    ...
    ' Set the range for reading
    myTXHSTL4.SetRange(In_ListNo, In_Year, In_Type, WrkDate, False)

    ' Loop through all records
    Do While Not myTXHSTL4.IsEOF
      myTXHSTL4.ReadFileE()
      If myTXHSTL4.IsEOF Then Exit Do
      ...
      If HstRcode = "I" Or HstRcode = "V" Then Continue Do
      If In_AsofDate < GetDBDate(myTXHSTL4._PDATE) Then Continue Do

      ' Accumulate totals
      result.TotRec += HstPamt
      If HstPencd = "BI" Then result.TotBondRec += HstPCamt

      ' Determine interest date
      HstIntDate = If(myTXHSTL4._CDATE < myTXHSTL4._PDATE,
                        GetDBDate(myTXHSTL4._CDATE),
                        GetDBDate(myTXHSTL4._PDATE))

      ' Determine installment number
      Dim WrkInstallNo As Integer = 0
      For i As Integer = 1 To 3
        If ProfPrPerd > i And HstIntDate > ProfTxGd(i) And WrkAdj(i) > 0 Then
          WrkInstallNo = i
        End If
      Next

      ' Process non-adjustment records
      If HstAdjcd <> "A" And HstAdjcd <> "R" Then
        ' Update interest collection
        If HstIamt > 0 Then
          For i As Integer = 0 To Math.Min(ProfPrPerd, 3)
            If HstIntDate > ProfTxGd(i) And Not result.IntCollect(i) Then
              result.IntCollect(i) = True
            End If
          Next
        End If
        ...
        If HstPamt > 0 Then
          result.IntDate(WrkInstallNo) = HstIntDate
          result.PrinPd(WrkInstallNo) = True
          result.IntPartialPaid = 0

          ' Determine method
          Select Case myTXHSTL4._CORC
            Case 1
              result.Method = "Cash"
            Case 2
              result.Method = If(myTXHSTL4._BATCHA = "W", "ECheck", "Check")
            Case 3
          ...
          result.OutIntDate = HstIntDate

          ' Update all installments if date is newer
          For i As Integer = 0 To 3
            If result.IntDate(i) < HstIntDate Then
              result.IntDate(i) = HstIntDate
              result.PrinPd(i) = True
            End If
          Next
        Else
          result.IntPartialPaid += HstIamt
        End If

        ' Update totals
        result.TotIntPaid += HstIamt
        result.LienPaid += HstLamt
    ...
    If result.IntPartialPaid < 0 Then
      result.IntPartialPaid = 0
    End If
    If result.LienPaid < 0 Then
      result.LienPaid = 0
    End If

    'Adjustment - Change interest date to last principal payment date
    If result.AdjFnd Then
      result.AdjFnd = False 'Reset flag
      WrkAdjAmt = 0
      WrkDate = 99999999
      result.IntPartialPaid = 0
      ...
      Do While Not myTXHSTL4.IsEOF
        ...
        If HstAdjcd <> "A" And HstAdjcd <> "R" And HstPamt > 0 And Not result.AdjFnd Then
          Return result
        End If
        result.AdjFnd = True
        result.IntDate(0) = ChkDate
        WrkAdjAmt = WrkAdjAmt + HstPamt
        result.AdjAmt = WrkAdjAmt
        If HstPamt = 0 And HstIamt <> 0 Then
          result.IntPartialPaid = result.IntPartialPaid + HstIamt
        End If
