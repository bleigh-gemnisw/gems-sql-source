...
#Region "Variables"
  ...
  'MK 7/27/26 Begin
  Dim mvarOut_IntCollect As Boolean
  Dim mvarOut_IntPaidInstall As Decimal
  'MK 7/27/26 Begin
  ...
  'end add ----------------------------------
  'MK 9/11/25 Begin
  Dim mvarOut_Adj1 As Decimal
  Dim mvarOut_Adj2 As Decimal
  Dim mvarOut_Adj3 As Decimal
  Dim mvarOut_Adj4 As Decimal
  'MK 9/11/25 End
...
#Region "Subroutines"
  ...
  Public Sub CalcInterest()
    ...
    'MK 8/1/25 Begin
    Dim InvCCReason As String
    'MK 8/1/25 End
    'MK 9/3/25 Begin
    Dim InvCCTxT As Decimal
    ...
    'MK 10/9/25 Begin
    WrkIntCollect(0) = False
    WrkIntCollect(1) = False
    WrkIntCollect(2) = False
    WrkIntCollect(3) = False
    'MK 10/9/25 End
    ...
    If myTXINV._ICODE = "D" Or myTXINV._ICODE = "E" Then
      'MK 9/29/25 End
   ...
   InvDist <> myTXPROF._DIST Then
    ...
    Else
      ...
      If myTXINV._ICODE = "E" Then
        WrkAdj(0) = InvCCTx1 + InvDefer1
        WrkAdj(1) = InvCCTx2 + InvDefer2
        WrkAdj(2) = InvCCTx3 + InvDefer3
        WrkAdj(3) = InvCCTx4 + InvDefer4
        sbDebug.Append(vbCrLf & "Defer Expired:" & InvDeferT)
      Else
        'MK 9/29/25 End
        WrkAdj(0) = InvCCTx1
        WrkAdj(1) = InvCCTx2
    ...
    'MK 8/1/25 Begin
    If MyCCAudit <> "" And MyCCAudit = InvCCReason And Year(InvCCDate) <> Year(ProfTxDt(0)) Then
      sbDebug.Append(vbCrLf & "C/C Audit")
      GoTo NextPerd
    End If
    'MK 8/1/25 End
...
ReadHist:
    ...
    If Not myTXHSTL4.IsEOF Then
      ...
      If HstIamt <> 0 Then
        ...
        End Select
        WrkIntPaid(WrkInstallNo) = WrkIntPaid(WrkInstallNo) + HstIamt
      End If
      If HstPCamt <> 0 Then
        If HstPencd = "BI" Then
...
NextPerd:
   ...
   Or myLowInt And WrkTPerc > 0 And In_Type = "P" And InvDist = 7 And In_Year = 2019 Then
    ...
    'MK 8/21/25 Begin
    If MyCCAudit <> "" And MyCCAudit = InvCCReason And Year(InvCCDate) = Year(ProfTxDt(0)) Then
      sbDebug.Append(vbCrLf & "CC Audit")
      WrkCCDt = DateAdd(DateInterval.Month, 1, WrkCCDtAdd)
      If In_IntDate <= WrkCCDt Then
        If WrkPaid(X) > 0 Then
    ...
    'MK 9/3/25 Begin
    If InvCCInt30 <> ChkDate And InvCCTxT > InvTaxT Then
    ...
    sbDebug.Append(vbCrLf & " Orig Interest: " & WrkIntOrig(X))
...
IntDone:
    ...
    'MK 7/27/26 Begin
    Out_IntCollect = WrkIntCollect(WrkInstallNo)
    Out_IntPaidinstall = WrkIntPaid(WrkInstallNo)
    'MK 7/27/26 End
    Out_Fee1 = 0
  ...
  Public Sub CalcInterest_219SW()
    ...
    Dim WrkAdj1 As Decimal
    Dim WrkAdj2 As Decimal
    Dim WrkAdj3 As Decimal
    Dim WrkAdj4 As Decimal
    Dim WrkInt1 As Decimal
    Dim WrkInt2 As Decimal
    Dim WrkInt3 As Decimal
    Dim WrkInt4 As Decimal
    Dim WrkIntOrig As Decimal
    ...
    Dim TmpInterest As Decimal
    Dim SaveIntCollect(3) As Boolean

    'added 10/7/25   mark and ken
    ...
    WrkInt3 = Out_Int3()
    WrkInt4 = Out_Int4()
    WrkIntOrig = Out_IntOrig()
    WrkIntOrig1 = Out_IntOrig1()
    WrkIntOrig2 = Out_IntOrig2()
    WrkIntOrig3 = Out_IntOrig3()
    WrkIntOrig4 = Out_IntOrig4()
    'added 10/7/25 mark and ken
    WrkPrin = Out_Prin
    WrkFee = Format(Out_Fee(), "standard")
    WrkLien = Format(Out_Lien(), "standard")
    WrkBond = Format(Out_Bond(), "standard")
    WrkTax = Format(Out_Prin(), "standard")
    WrkDue = Format(Out_Tot(), "standard")
    WrkGracePeriod = Out_GracePeriod
    'end add 10/7/25 mark and ken
    SaveIntCollect(0) = WrkIntCollect(0)
    'MK 6/4/26 Begoin
    'SaveIntCollect(1) = WrkIntCollect(0)
    'SaveIntCollect(2) = WrkIntCollect(0)
    'SaveIntCollect(3) = WrkIntCollect(0)
    SaveIntCollect(1) = WrkIntCollect(1)
    SaveIntCollect(2) = WrkIntCollect(2)
    SaveIntCollect(3) = WrkIntCollect(3)
    'MK 6/4/26 End

    If WrkIntOrig1 > 0 And WrkInt1 <> WrkIntOrig1 Or WrkIntOrig2 > 0 And WrkInt2 <> WrkIntOrig2 Or
      WrkIntOrig3 > 0 And WrkInt3 <> WrkIntOrig3 Or WrkIntOrig4 > 0 And WrkInt4 <> WrkIntOrig4 Then
      In_Type = WrkType2
      CalcInterest()
      'MK 10/9/25 SaveIntcollect has to be false before charging minimum interest
      If Not SaveIntCollect(0) Then
    ...
    If WrkAdj1 = 0 Then
      TmpInterest = WrkInt1
    Else
      TmpInterest = WrkIntOrig1
    End If
    'MK 6/4/26 Begin
    If SaveIntCollect(0) Or WrkIntCollect(0) Then
      TmpInterest = WrkIntOrig1
    End If
    'MK 6/4/26 End
    If WrkAdj2 = 0 Then
      TmpInterest = TmpInterest + WrkInt2
    Else
      TmpInterest = TmpInterest + WrkIntOrig2
    End If
    If WrkAdj3 = 0 Then
      TmpInterest = TmpInterest + WrkInt3
    Else
      TmpInterest = TmpInterest + WrkIntOrig3
    End If
    If WrkAdj4 = 0 Then
      TmpInterest = TmpInterest + WrkInt4
    Else
      TmpInterest = TmpInterest + WrkIntOrig4
    End If
    'added 10/7/25 mark and ken
    Out_Prin = WrkPrin
    Out_Fee = WrkFee
    ...
    Out_Bond = WrkBond
    Out_Prin = WrkTax
    Out_Tot = Round(TmpInterest + Out_Prin + Out_Fee + Out_Lien + Out_Bond, 2)
    Out_GracePeriod = WrkGracePeriod
    'end add 10/7/25 mark and ken
    Out_Int = TmpInterest
  End Sub
...
#Region "Properties"
  ...
  End Property
  'MK 7/27/26 Begin
  Public Property Out_IntCollect() As Boolean
    Get
      Out_IntCollect = mvarOut_IntCollect
    End Get
    Set(ByVal Value As Boolean)
      mvarOut_IntCollect = Value
    End Set
  End Property
  Public Property Out_IntPaidInstall() As Decimal
    Get
      Out_IntPaidInstall = mvarOut_IntPaidInstall
    End Get
    Set(ByVal Value As Decimal)
      mvarOut_IntPaidInstall = Value
    End Set
  End Property
  'MK 7/27/26 End
  ...
  End Property
  'MK 9/11/25 Begin
  Public Property Out_Adj1() As Decimal
    Get
      Out_Adj1 = mvarOut_Adj1
    End Get
    Set(ByVal Value As Decimal)
      mvarOut_Adj1 = Value
    End Set
  End Property
  Public Property Out_Adj2() As Decimal
    Get
      Out_Adj2 = mvarOut_Adj2
    End Get
    Set(ByVal Value As Decimal)
      mvarOut_Adj2 = Value
    End Set
  End Property
  Public Property Out_Adj3() As Decimal
    Get
      Out_Adj3 = mvarOut_Adj3
    End Get
    Set(ByVal Value As Decimal)
      mvarOut_Adj3 = Value
    End Set
  End Property
  Public Property Out_Adj4() As Decimal
    Get
      Out_Adj4 = mvarOut_Adj4
    End Get
    Set(ByVal Value As Decimal)
      mvarOut_Adj4 = Value
    End Set
