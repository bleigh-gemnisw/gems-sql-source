...
#End Region
  ...
  Sub AddOneRecord()
    ...
    Select Case WrkType
      ...
      Case "R"
        ...
        End If
      Case "S"
        MyTXSUPPC.GetOneRecordP(ListNo)
        If Not MyTXSUPPC.RecordNotFound Then
          WrkName = Trim(MyTXSUPPC._NAME)
        End If
    End Select
  ...
  Public Sub RemoveData()
    ...
    If DataGrdView.SelectedRows.Count > 0 Then
      'MK 9/29/25 Remove caused error, row shouldn't be an integer
      '  For Each row In DataGrdView.SelectedRows
      '    myds.Tables(0).Rows(row).Delete()
      '    Exit For
      '  Next
      For Each row As DataGridViewRow In DataGrdView.SelectedRows
        myds.Tables(0).Rows(row.Index).Delete()
      Next
      'MK 9/29/25
    End If
  End Sub
