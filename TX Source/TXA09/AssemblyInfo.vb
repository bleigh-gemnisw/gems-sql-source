...
'MK 6/ 9/25 Change TXINV to use RunUpdateQuery and TXHST to InsertOneRecord 
'MK 6/19/25 Make Open Batch, Adjustments and Totals SaveData async processing
'MK 6/26/25 Disable async processing (comment out)
'MK 7/ 3/25 Kensington: use CalcInterest_219SW for S & W types
'MK 7/ 8/25 Fix syntax error with BONT update
'MK 7/ 9/25 Fastpath-run formatgrid and populate find box
'MK 7/16/25 Kensington: CalcInterest_219SW - If there is no Type2 then use normal Interest 
'MK 7/21/25 PostBatch: Allow retry or cancel update if it fails
'MK 7/22/25 Kensington: CalcInterest_219SW - Recalc Total Due 
'MK 7/23/25 Kensington: CalcInterest_219SW - Change when to not look at other type
'MK 7/28/25 MovetoTXINV: Allow retry or cancel update if it fails
'MK 7/31/25 Change Selected items limit from 25 to 100
'MK 8/14/25 Change all TXINV UpdateOneRecord to only update needed fields. Fix Current Fee/Bond on adjustments
'MK 9/25/25 Add CC Billdate, Show CC number on before bill
'MK 9/29/25 Add Defer Expired logic (ICODE=E)
'MK 10/2/25 Add Baldue logic for deferral for edit/posting
'MK  6/4/26 Kensington: CalcInterest_219SW check if interest collected then use OrigInterest   
'MK  7/2/26 Change search default records from 50 to 25, run formatgrid only once at startup 
'MK  7/8/26 Add Grace Dates to Winchester Dup Bills (Tax) 
'MK 7/13/26 Kensington: Change CalcInterest_219 to charge min interest if other type is 0 due and paid no interest 
'MK 7/20/26 Kensington: Change CalcInterest_219 to charge min interest if other type has no bill 
'MK 7/23/26 Southington: Dupbill-US hide installments based on TXPROF due dates
'MK 7/27/26 Move CASHINT & SW to common2 
'MK 8/24/26 Add AppConfig, use to determine if username or computername is used for XML file name. Change Balance Due calc when BALTL is on.
'MK 9/ 1/26 WebBrowser: Use WebView2 in private mode then delete cache afterwards
'MK 9/14/26 Add ResetTM675 code and button
Imports System
