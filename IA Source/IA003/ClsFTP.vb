...
'            End If
    ...
    '
    ' Get the size of the file on the FTP Server.
    Public Function GetFileDate(ByVal sFileName As String, ByVal GMTOffset As Double) As String
        Dim FileDate As String
        Dim FileDateMDY As String
        Dim AdjustedDateMDY As String
        Dim WrkDate As Date
...
'        If (m_iRetValue = 213) Then
            FileDate = m_sReply.Substring(4)
'        Else
'            MessageString = m_sReply
'            Throw New IOException(m_sReply.Substring(4))
'        End If
        
        FileDateMDY = Val(Mid(FileDate, 5, 2)) & "/" & Val(Mid(FileDate, 7, 2)) & "/" & Mid(FileDate, 1, 4) & _
          " " & Mid(FileDate, 9, 2) & ":" & Mid(FileDate, 11, 2) & ":" & Mid(FileDate, 13, 2)

        WrkDate = FileDateMDY
        WrkDate = DateAdd(DateInterval.Hour, GMTOffset, WrkDate)

        AdjustedDateMDY = Format(WrkDate, "M/d/yyyy H:mm:ss")
        Return AdjustedDateMDY
    End Function
