...
Module SFTPRebex
  ...
  Public Sub CheckFiles(ByRef Cancel As Boolean)
    ...
    Dim WrkFileList As SftpItemCollection
    Dim WrkItem As Rebex.Net.SftpItem
    ...
    If MyDebug Then
      ...
      WrkSFTP.LogWriter = New Rebex.FileLogWriter(WrkDatapath & "rebexlog.txt", Rebex.LogLevel.Debug)
    End If

    Try
      WrkSFTP.Connect(cHostIP) ', cPort)
    Catch
    End Try
    ...
    WrkSFTP.ChangeDirectory(cDirRoot)
    WrkSFTP.ChangeDirectory(MyFTPDir)
    WrkFileList = WrkSFTP.GetList()
    WrkFileList.Sort()
    J = 0
    ...
    myFrmProgress.Refresh()
    Application.DoEvents()
    WrkSFTP.ChangeDirectory("Reports")
    WrkFileList = WrkSFTP.GetList()
    WrkFileList.Sort()
    For Each WrkItem In WrkFileList
      WrkName = WrkItem.Name '.ToUpper()
    ...
    myFrmProgress.Refresh()
    Application.DoEvents()
    WrkSFTP.ChangeDirectory("..")
    WrkSFTP.ChangeDirectory("GemsHelp")
    WrkFileList = WrkSFTP.GetList()
    WrkFileList.Sort()
    For Each WrkItem In WrkFileList
      WrkName = WrkItem.Name ' .ToUpper()
    ...
    Application.DoEvents()
    WrkSFTP.ChangeDirectory("..")
    WrkSFTP.ChangeDirectory(MyTownNo)
    WrkSFTP.ChangeDirectory("Reports")
    WrkFileList = WrkSFTP.GetList()
    WrkFileList.Sort()
    For Each WrkItem In WrkFileList
    ...
    If MyCustom <> String.Empty Then
      ...
      Application.DoEvents()
      WrkSFTP.ChangeDirectory("..")
      WrkSFTP.ChangeDirectory("Reports" & MyCustom)
      WrkFileList = WrkSFTP.GetList()
      WrkFileList.Sort()
      For Each WrkItem In WrkFileList
        WrkName = WrkItem.Name ' .ToUpper()
...
NextMissing:
    ...
    For I = 0 To WrkRemoteName.GetUpperBound(0)
      ...
      If drSel.GetUpperBound(0) = -1 Then
        'EXE & CONFIG must be installed app in order to add it. Any DLL, RPT or CHM can be added.
        PosApp = -1
  ...
  Public Sub ApplyFiles()
    ...
    End Try
    WrkSFTP.ChangeDirectory(MyFTPDir)
    '    WrkSFTP.TransferMode = FtpTransferMode.Zlib
    WrkSFTP.TransferType = FtpTransferType.Binary
      ...
      With ds.Tables(0).Rows(I)
        If .Item("FileType") = "Program" Then
          ...
          If .Item("RemoteSize") > 0 Then
            ...
            End If
            Try
              WrkSFTP.GetFile(.Item("FileName"), WrkFile)
            Catch
              Exit Sub
            End Try
            If MyAutomate Then
  ...
  Public Sub HotlineUpdate(ByRef Cancel As Boolean)
    ...
    If Not WrkSFTP.FileExists(WrkHotline) Then
    ...
    WrkSFTP.GetFile(WrkHotline, WrkFile)
    WrkFileInfo = My.Computer.FileSystem.GetFileInfo(WrkFile)
    WrkDate = WrkSFTP.GetFileDateTime(WrkHotline)
  ...
  Public Sub DirectoryUpdate(ByVal DirName As String, ByRef Cancel As Boolean)
    ...
    WrkSFTP.ChangeDirectory(cDirRoot)
    WrkSFTP.ChangeDirectory(MyFTPDir)
    WrkSFTP.ChangeDirectory(DirName)
    WrkSFTP.TransferType = FtpTransferType.Binary
    ...
    For Each WrkItem In WrkFileList
      ...
      If WrkRemoteSize > 0 Then
        ...
        If WrkRemoteDate(I) <> WrkLocalDateStr Then
          ...
          If WrkRemoteDate(I) <> Format(WrkLocalAdjDate, "M/d/yyyy H:mm") Then
            WrkSFTP.GetFile(WrkName, WrkFile)
            WrkFileInfo = My.Computer.FileSystem.GetFileInfo(WrkFile)
