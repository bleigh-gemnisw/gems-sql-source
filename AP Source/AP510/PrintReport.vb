...
Module PrintReport
  ...
  ' Begin Change Ken 9/10/26 - Use Pay To name when entered; otherwise use Vendor name
  Dim WrkNamePay As String
  ' End Change Ken 9/10/26
  ...
  Public Sub PrtReport()
    ...
    myCKHISTQ = New CKHISTQ.myData()
    myCKHISTQ.MyDBConn = myDBConnect
...
Done:
  ...
  Private Sub GetDetailAP()
    Dim sw As StreamWriter = New StreamWriter(MyFrmAP510B.LblFilePath.Text)
    ...
    If MyFrmAP510B.TxtChkFrom.Text <> String.Empty Then
      WrkQry = "PAYBN = " & MyUtils.Quo(WrkBank) & WrkAnd & "PAYCK >=" & MyUtils.CnvSng(MyFrmAP510B.TxtChkFrom.Text) & WrkAnd & "PAYCK <=" & MyUtils.CnvSng(MyFrmAP510B.TxtChkTo.Text)
    Else
    ...
    If MyFrmAP510B.RbAPCSV.Checked Then
...
ReadNext:
    ...
    If Not myAPERCNQ.IsEOF Then
      ...
      ' Begin Change Ken 9/10/26 - Set name used for check reconciliation output/report
      WrkNamePay = String.Empty
      If Not myVENDOR.RecordNotFound Then
        If Trim(myVENDOR._PYNAM) <> String.Empty Then
          WrkNamePay = Trim(myVENDOR._PYNAM)
        Else
          WrkNamePay = Trim(myVENDOR._VENNM)
        End If
      End If
      ' End Change Ken 9/10/26
      If MyFrmAP510B.RbAP.Checked Then
        sw.WriteLine(WriteAPFile(WrkAcct))
      End If
      If MyFrmAP510B.RbAPTD.Checked Then
        sw.WriteLine(WriteAPTDFile(WrkAcct))
      End If
      If MyFrmAP510B.RbAPBOA.Checked Then
        sw.WriteLine(WriteAPBOAFile(WrkAcct))
      End If
      If MyFrmAP510B.RbAPBOAShort.Checked Then
        sw.WriteLine(WriteAPBOAShortFile(WrkAcct))
      End If
      If MyFrmAP510B.RbAPWebster.Checked Then
        sw.WriteLine(WriteAPWebster(WrkAcct))
      End If
      If MyFrmAP510B.RbAPCSV.Checked Then
        sw.WriteLine(WriteAPCSV(WrkAcct))
      End If
      If MyFrmAP510B.RbAPIon.Checked Then
        sw.WriteLine(WriteAPIon(WrkAcct))
      End If
      ...
      With myAPERCNQ
        ...
        If Not myVENDOR.RecordNotFound Then
          'dr.Item("name") = Trim(myVENDOR._VENNM) ' Change Ken 9/10/26
          dr.Item("name") = WrkNamePay ' Change Ken 9/10/26
        Else
...
NextRec:
  ...
  Private Function WriteAPBOAFile(ByVal WrkAcct As String) As String
    ...
    With myAPERCNQ
      ...
      If myVENDOR.RecordNotFound Then
        sb.Append(._VNDNR)
      Else
        'sb.Append(Trim(myVENDOR._VENNM)) ' Change Ken 9/10/26
        sb.Append(WrkNamePay) ' Change Ken 9/10/26
      End If
    End With
    Return sb.ToString
  ...
  Private Function WriteAPTDFile(ByVal WrkAcct As String) As String
    ...
    With myAPERCNQ
      ...
      Else
        'sb.Append(MyUtils.JustifyLeft(Trim(myVENDOR._VENNM), 30)) ' Change Ken 9/10/26
        sb.Append(MyUtils.JustifyLeft(WrkNamePay, 30)) ' Change Ken 9/10/26
      End If
      WrkDate = MyUtils.GetDBDate(._PAYP8)
      sb.Append(Format(WrkDate.Month, "00"))
      sb.Append(Format(WrkDate.Day, "00"))
      sb.Append(Right(WrkDate.Year, 2))
      sb.Append("0")
  ...
  Private Function WriteAPWebster(ByVal WrkAcct As String) As String
    ...
    Else
      With myVENDOR
        If Trim(._PYNAM) = "" Then
          'WrkName = Trim(._VENNM) ' Change Ken 9/10/26
          WrkName = WrkNamePay ' Change Ken 9/10/26
          'WrkFlipName = DoFlipName(Trim(._VENNM)) ' Change Ken 9/10/26
          WrkFlipName = DoFlipName(WrkNamePay) ' Change Ken 9/10/26
          AddrLine = SetVndrAddrLine(._VADD1, ._VADD2, ._VADD3,
         ._VADD4, ._VZIP, ._VZIPE)
        Else
          'WrkName = Trim(._PYNAM) ' Change Ken 9/10/26
          WrkName = WrkNamePay ' Change Ken 9/10/26
          'WrkFlipName = DoFlipName(Trim(._PYNAM)) ' Change Ken 9/10/26
          WrkFlipName = DoFlipName(WrkNamePay) ' Change Ken 9/10/26
  ...
  End Function
  Private Function WriteAPIon(ByVal WrkAcct As String) As String
    Dim sb As StringBuilder
    Dim WrkName As String
    Dim WrkFlipName As String
    ...
    With myAPERCNQ
      sb.Append(._PAYCK)
      sb.Append(",")
      sb.Append(Format(._PAYAM, "Fixed"))
      sb.Append(",")
      sb.Append(Format(MyUtils.GetDBDate(._PAYP8), "MM/dd/yyyy"))
      sb.Append(",")
    End With
    If myVENDOR.RecordNotFound Then
      sb.Append("")
  ...
  Private Sub GetDetailPR()
    ...
    If MyFrmAP510B.TxtChkFrom.Text <> String.Empty Then
      WrkByCheckNo = True
      WrkQry = "CKNUM >=" & MyUtils.CnvSng(MyFrmAP510B.TxtChkFrom.Text) & WrkAnd & "CKNUM <=" & MyUtils.CnvSng(MyFrmAP510B.TxtChkTo.Text)
    Else
      WrkByCheckNo = False
    End If
    If WrkPrData <> "*" Then
    ...
    If MyFrmAP510B.RbPRCSV.Checked Then
    ...
    If Not myCKHISTQ.IsEOF Then
      ...
      With myCKHISTQ
        If Not WrkByCheckNo Then
          ...
          If WrkDate < MyFrmAP510B.DtPckFrom.Value Then
            GoTo NextRec
          End If
          If WrkDate > MyFrmAP510B.DtPckTo.Value Then
            GoTo NextRec
          End If
        End If
      End With
      ...
      With myAPEBNK
        WrkAcct = Trim(._BNKAC)
      End With
      'If MyFrmAP510B.RbPRTD.Checked Then
      '  sw.WriteLine(WritePRTDFile(WrkAcct))
      'End If
      'If MyFrmAP510B.RbPRBOA.Checked Then
      '  sw.WriteLine(WritePRBOAFile(WrkAcct))
      'End If
      'If MyFrmAP510B.RbPRBOAShort.Checked Then
      '  sw.WriteLine(WritePRBOAShortFile(WrkAcct))
      'End If
      'If MyFrmAP510B.RbPRWebster.Checked Then
      '  sw.WriteLine(WritePRWebster(WrkAcct))
      'End If
      If MyFrmAP510B.RbPRCSV.Checked Then
        sw.WriteLine(WritePRCSV(WrkAcct))
      End If
      If MyFrmAP510B.RbPRIon.Checked Then
        sw.WriteLine(WritePRIon(WrkAcct))
      End If
...
NextRec:
  ...
  End Function
  Private Function WritePRIon(ByVal WrkAcct As String) As String
    Dim sb As StringBuilder
    Dim WrkFlipName As String
    ...
    With myCKHISTQ
      sb.Append(._CKNUM)
      sb.Append(",")
      sb.Append(Format(._CKAMT, "Fixed"))
      sb.Append(",")
      sb.Append(Format(._CHKDTE, "##/##/####"))
      sb.Append(",")
      WrkFlipName = DoFlipName(Trim(._EMNAME))
      sb.Append(WrkFlipName)
    End With
