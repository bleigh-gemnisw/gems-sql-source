Module PrintEdits
  ...
  Dim EncPOAcct(250) As String
  Dim EncAmt(250) As Decimal
  ...
  Sub WriteDS(ByVal WrkBatch As Integer, ByVal Post As Boolean)
    ...
    Dim WrkEncAmt As Decimal
    Dim WrkPrevEncAmt As Decimal
    ...
    Array.Clear(EncPOAcct, 0, 250)
    Array.Clear(EncAmt, 0, 250)
...
ReadNext:
      ...
      With myAPEBCHQ
        ...
        For I = 0 To dsFile2.Tables(0).Rows.Count - 1
          ...
          Else
            ...
            dr("acct") = WrkAcct
            dr("acctdesc") = "*** Invalid Fund ***"
            dr("errmsg") = "*** Invalid Fund ***"
          ...
          If ._PONBR > 0 And ._POLIQ = "" Then
            ...
            WrkPrevEncAmt = 0
            WrkPOOpn = 0
            L = LookupEncAcct(._FSCYR & ._PONBR & WrkAcct)
            If EncAmt(L) <> 0 Then
              WrkPrevEncAmt = EncAmt(L)
            End If
            dr = ds2.Tables(0).NewRow
            dr("bchno") = WrkBatch
            dr("bchdate") = MyUtils.GetDBDate(myBCHHDR._PSDT)
            ...
            For J = 0 To dsSum.Tables(0).Rows.Count - 1
              ...
              End If
            Next
            WrkPOOpn = WrkPOOpn - WrkPrevEncAmt
            If WrkPOOpn < 0 Then WrkPOOpn = 0
            If ._LEOPN = "P" Then
              If dsFile2.Tables(0).Rows(I).Item("wrksum") <= WrkPOOpn Then
                WrkEncAmt = dsFile2.Tables(0).Rows(I).Item("wrksum")
              Else
                WrkEncAmt = WrkPOOpn
              End If
            Else
              WrkEncAmt = WrkPOOpn
            End If
            FundCtlAmt(K) = FundCtlAmt(K) + WrkEncAmt
            If WrkEncAmt > 0 Then
              dr("credit") = WrkEncAmt
              dr("debit") = 0
            Else
              dr("credit") = 0
              dr("debit") = Math.Abs(WrkEncAmt)
            End If
            EncPOAcct(L) = ._FSCYR & ._PONBR & WrkAcct
            EncAmt(L) = EncAmt(L) + WrkEncAmt
            dr("acct") = WrkAcct
              If Not myGLACCT.RecordNotFound Then
                dr("acctdesc") = myGLACCT._GLDSC
                dr("errmsg") = String.Empty
  ...
  End Function
  Private Function LookupEncAcct(ByVal Acct As String) As Integer
    Dim I As Integer

    For I = 0 To EncPOAcct.GetUpperBound(0)
      If EncPOAcct(I) & "" = "" Then
        Return I
      End If
      If Acct = EncPOAcct(I) Then
        Return I
      End If
    Next
    Return 0
