...
Module PrintReadings
  ...
  Public Sub PrtReadings()
    ...
    Else
      If myTOWN._TOWNBR = 131 Then 'Southington
        WrkReadDate1 = WrkReadDateQ1
      End If
    End If
...
Done:
  ...
  Private Sub GetDetail(ByVal WrkFile As String, ByVal WrkReadDate As Date, ByVal WrkQtr As Integer)
    ...
    If WrkCompany = "Other" Then 'Skip Heading
      If myTOWN._TOWNBR = 280 Then 'Norfolk
    ...
    Select Case WrkCompany
      ...
      Case "Other"
        Select Case myTOWN._TOWNBR
          Case = 131 'Southington
            WrkXref = Mid(strBuffer, 1, 9) 'Meter Number
            WrkXref = Replace(WrkXref, "-", ".")
            WrkReadDate = Mid(strBuffer, 374, 10) 'Read Date
            WrkReading = MyUtils.CnvSng(Mid(strBuffer, 386, 6)) 'Reading
            WrkName = ""
            WrkLocation = ""
...
Cleanup:
  ...
  ByVal WrkReading As Integer, ByVal WrkName As String, ByVal WrkLocation As String)
    ...
    If WrkAcct = 0 Then
      ...
      myUTCUSTMT.GetOneRecordP(WrkAcct, "", MyUtils.SetDBDate(WrkReadDate))
      With myUTCUSTMT
    ...
    If Not Found Then
      ...
      'MK 7/17/25 End
      If WrkName = "" Then
        WrkName = Trim(myUTCUST._CUNAM1)
        WrkLocation = Trim(myUTCUST._CULOCNO) & " " & Trim(myUTCUST._CULOC)
      End If
...
NextRec:
    ...
    myUTCUSTQ.CloseFile()
  End Sub
  Public Function GetDBDateMDY6(ByVal DateIn As Integer) As Date
    Dim WrkDate As Date
    ...
    If WrkLen = 6 Then
      Try
        WrkDate = Left$(StrDate, 2) & "/" & Mid$(StrDate, 3, 2) & "/" & Right$(StrDate, 2)
    ...
    If WrkLen = 5 Then
      Try
        WrkDate = Left$(StrDate, 1) & "/" & Mid$(StrDate, 2, 2) & "/" & Right$(StrDate, 2)
      Catch ex As Exception
        Return #1/1/1800#
      End Try
    End If
