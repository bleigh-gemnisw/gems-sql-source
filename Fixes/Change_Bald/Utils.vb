...
Private Sub GlobalErrorLog(ByVal ex As Exception, ByVal ErrType As String)
    ...
    Dim sw As System.IO.StreamWriter = New System.IO.StreamWriter(WrkPath & MyUserID & _
    ...
    sw.Close()
    MsgBox("Program (" & ErrType & ") ended abnormally. Send " & MyUserID & "-" & _
...
Public Function GetDataPath() As String
   ...
   End With
    ...
    Return WrkFileName
End Function
Public Function GetReportPath(ByVal RptName As String) As String
		...
		WrkTownNbr = Format(dsTown.Tables(0).Rows(0).Item("townbr"), "000")
		WrkFileName = System.Reflection.Assembly.GetExecutingAssembly.Location
		Dim myFileVersionInfo As FileVersionInfo = FileVersionInfo.GetVersionInfo(WrkFileName)
	 ...
	 End With
		...
		If Not WrkExists Then
			WrkFileName = Replace(WrkFileName, WrkTownNbr & "\", "")
		End If
...
Public Sub PrtScreen(ByVal FormName As Form)
  ...
  WrkTimestamp = Format(Date.Now, "MMddyyyy HHmmss")
  WrkPrtFileName = WrkDataPath & "PrtScrn " & MyUserID & " " & WrkTimestamp & ".jpg"
