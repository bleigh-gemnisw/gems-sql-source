...
Public Sub GetOneRecordP(ByVal Code As String)
  ...
  StrSQL = "Select * from " & cFileName & " where VNDNR='" & Code & "'"
  Try
    Conn = MyDBConn.Open
  ...
  End Try
End Sub
Public Function GetViewbyVendNo(ByVal Code As String, ByVal NumRecs As Integer) As DataSet
  Dim Conn As SqlConnection
  Dim objCommand As SqlCommand
  Dim ds As DataSet = New DataSet

  If NumRecs > 0 Then
    StrSQL = "Select vndnr, vennm from " & cFileName & " where VNDNR>='" & Code & "'"
  Else
    StrSQL = "Select top " & NumRecs & " vndnr, vennm from " & cFileName & _
      " where VNDNR>='" & Code & "' and ACREC<>'S'"
  End If
  Conn = MyDBConn.Open
  objCommand = New SqlCommand(StrSQL, Conn)
...
Public Function PosData(ByVal Code As String) As DataSet
  ...
  StrSQL = "Select * from " & cFileName & " where VNDNR>='" & Code & "' order by vndnr"
  Conn = MyDBConn.Open
  objCommand = New SqlCommand(StrSQL, Conn)
