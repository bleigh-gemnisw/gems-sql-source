...
Public Class MyData
  ...
  Const cFileName2 As String = "UTCUST"
#Region "Constructors"
...
#Region "Methods: File Access Routines"
  ...
  Public Function GetAllList(ByVal WrkListNo As Integer) As DataSet
    ...
    If ds.Tables(0).Rows.Count > 0 Then
      StrSQL = "Select CUACCT,CUNAM1,CUNAM2,CUADD1,CUADD2,CUCITY,CUST,CUZIP,CUMAD1,CUMAD2,CUMCTY,CUMST,CUMZIP,CUTEL#," &
      "CUDST,CUPHAS,CUADDX,CUTIE,CUMAP,CUVOLM,CUPAGE,CUZONE,CUPCAT,CUXREF,CUMSIZ,CYC,OID,CUSERN,CUROUT,CUMETN," &
      "CUMETP,CUREGN,CULOC#,CULOC,CUCNT#,CUAPL#,CUFUND,CUSECT,CUSDES" &
       " from " & cFileName2 & " where cuacct=" & WrkListNo
      Conn = MyDBConn.Open
      objCommand = New SqlCommand(StrSQL, Conn)
      da = New SqlDataAdapter
      da.SelectCommand = objCommand
      da.Fill(ds2, cFileName)
      objCommand = Nothing
      Conn.Close()
      If ds2.Tables(0).Rows.Count > 0 Then
        ds2.Tables(0).Columns.Add("LOGCMT", GetType(String))
        ds2.Tables(0).Columns.Add("LOGDTE", GetType(Decimal))
        ds2.Tables(0).Columns.Add("LOGTIM", GetType(Decimal))
        ds2.Tables(0).Rows(0).Item("LOGCMT") = "Current Record"
        ds2.Tables(0).Rows(0).Item("LOGDTE") = SetDBDate(Date.Now)
        ds2.Tables(0).Rows(0).Item("LOGTIM") = Date.Now.Hour & Format(Date.Now.Minute, "00") & Format(Date.Now.Second, "00")
        ds.Merge(ds2)
      End If
    End If
    Return ds
