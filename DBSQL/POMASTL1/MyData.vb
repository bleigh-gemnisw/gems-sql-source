...
Public Function GetAllPONo(ByVal WrkFscyr As Integer, ByVal WrkPonbr As Integer, _
 ByVal NumRecs As Integer) As DataSet
  ...
  End Try
End Function
Public Function GetSumbyAcct(ByVal WrkFscyr As Integer, ByVal WrkPONbr As Integer, _
 WrkPOSuf As Integer, WrkPODate As Integer) As DataSet
  Dim Conn As SqlConnection
  Dim objCommand As SqlCommand
  Dim ds As DataSet = New DataSet
  Dim WrkTop As String
  ...
  RecordNotFound = False
    If WrkPODate > 0 Then
      StrSQL = "select fdnbr,sfund,dpnbr,obnbr,fnpgm,subfn,sum(exval) as wrksum from " & cFileName _
   & " where fscyr=" & WrkFscyr & " and ponbr=" & WrkPONbr & " and posuf=" & WrkPOSuf _
   & " and popst=" & WrkPODate & " and fdnbr>0 group by fdnbr,sfund,dpnbr,obnbr,fnpgm,subfn"
    Else
      StrSQL = "select fdnbr,sfund,dpnbr,obnbr,fnpgm,subfn,sum(exval) as wrksum from " & cFileName _
   & " where fscyr=" & WrkFscyr & " and ponbr=" & WrkPONbr & " and posuf=" & WrkPOSuf _
   & " and fdnbr>0 group by fdnbr,sfund,dpnbr,obnbr,fnpgm,subfn"
    End If
    Try
        Conn = MyDBConn.Open
        objCommand = New SqlCommand(StrSQL, Conn)
  ...
  End Try
End Function
Public Function GetViewbyVendorLC(ByVal WrkVndnr As String, ByVal WrkFscyr As Integer, _
 ByVal WrkPonbr As Integer, ByVal NumRecs As Integer) As DataSet
  ...
  If NumRecs > 0 Then
    WrkTop = "TOP " & NumRecs & " "
  End If
  StrSQL = "Select " & WrkTop & "fscyr, ponbr, posuf,vndnr,vennm,amtnt,popen,cmpcd,ractd from " & cFileName _
    & " where vndnr='" & WrkVndnr & "' and fscyr =" & WrkFscyr & " and ponbr>=" & WrkPonbr & " And poseq=0 Or " _
    & " vndnr='" & WrkVndnr & "' and fscyr >" & WrkFscyr & " And poseq=0 order by vndnr,fscyr,ponbr,posuf"
  Try
    Conn = MyDBConn.Open
    objCommand = New SqlCommand(StrSQL, Conn)
    'Fill the dataset with the data
  ...
  End Try
End Function
Public Function GetViewbyPOL6(ByVal WrkFscyr As Integer, ByVal WrkPonbr As Integer, _
 ByVal NumRecs As Integer) As DataSet
  Dim Conn As SqlConnection
  Dim objCommand As SqlCommand
  Dim ds As DataSet = New DataSet
  ...
  If NumRecs > 0 Then
    WrkTop = "TOP " & NumRecs & " "
  End If
  StrSQL = "Select " & WrkTop & "fscyr, ponbr, poseq,rqqty,itnbr,itdsc,unitp,unmsr,exval," & _
   "fdnbr,sfund,dpnbr,obnbr,fnpgm,subfn from " & cFileName _
    & " where fscyr =" & WrkFscyr & " and ponbr=" & WrkPonbr & " and poseq>0 " _
    & " order by fscyr,ponbr,fdnbr,sfund,dpnbr,obnbr,fnpgm,subfn,posuf"
  Try
    Conn = MyDBConn.Open
    objCommand = New SqlCommand(StrSQL, Conn)
    'Fill the dataset with the data
  ...
  End Try
End Function
Public Function GetViewbyPOL8(ByVal WrkLlocn As String, ByVal WrkFscyr As Integer, _
 ByVal WrkPonbr As Integer, ByVal WrkPrtFg As Boolean, ByVal NumRecs As Integer) As DataSet
  Dim Conn As SqlConnection
  Dim objCommand As SqlCommand
  Dim ds As DataSet = New DataSet
  ...
  If NumRecs > 0 Then
    WrkTop = "TOP " & NumRecs & " "
  End If
  StrSQL = "Select " & WrkTop & "fscyr, ponbr, posuf,vndnr,vennm,amtnt,popst from " & cFileName _
    & " where ponbr>=" & WrkPonbr & " and poseq=0"
  If WrkFscyr > 0 Then
    StrSQL = StrSQL & " and fscyr=" & WrkFscyr
  End If
  If WrkLlocn <> "" Then
    StrSQL = StrSQL & " and llocn='" & WrkLlocn & "'"
  End If
  If WrkPrtFg Then
    StrSQL = StrSQL & " and prtfg<>'Y'"
  End If
  StrSQL = StrSQL & " order by fscyr,ponbr,posuf,vndnr"
  Try
    Conn = MyDBConn.Open
    objCommand = New SqlCommand(StrSQL, Conn)
    'Fill the dataset with the data
...
Public Function ReplaceDS(ByVal ds As DataSet) As DataSet
    ...
    With myTable
      ...
      .Columns.Add("posuf", Type.GetType("System.Int32"))
  ...
  For I = 0 To ds.Tables(0).Rows.Count - 1
    With ds.Tables(0).Rows(I)
      ...
      dr.Item("posuf") = .Item("posuf")
      dr.Item("vndnr") = .Item("vndnr")
      dr.Item("vennm") = .Item("vennm")
      dr.Item("amtnt") = .Item("amtnt")
      dr.Item("wkdate") = .Item("popst")
