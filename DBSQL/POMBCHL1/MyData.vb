...
Public Function GetViewbyBatch(ByVal WrkBchno As Integer, ByVal NumRecs As Integer) As DataSet
  ...
  If NumRecs > 0 Then
    WrkTop = "TOP " & NumRecs & " "
  End If
  StrSQL = "Select " & WrkTop & "ponbr, vennm,amtgr from " & cFileName & _
  ...
  End Try
End Function
Public Function IsBatchHold(ByVal WrkBchno As Integer) As Boolean
  Dim Conn As SqlConnection
  Dim objCommand As SqlCommand
  Dim ds As DataSet = New DataSet
  ...
  RecordNotFound = False
  StrSQL = "Select top 1 ponbr from " & cFileName & _
    " where bchno=" & WrkBchno & " and cmpcd='H'"
  Try
    Conn = MyDBConn.Open
    objCommand = New SqlCommand(StrSQL, Conn)
    'Fill the dataset with the data
