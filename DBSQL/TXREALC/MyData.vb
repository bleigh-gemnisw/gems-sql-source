...
End Sub
 ...
 ClearFields 
  ...
  End Try
End Sub
  Public Function PosData(ByVal Wrklistno As Integer, Optional ByVal Fields As String = "") As DataSet
    Dim Conn As SqlConnection
    Dim objCommand As SqlCommand
    Dim ds As DataSet = New DataSet

    If Fields <> "" Then
      StrSQL = "Select " & Fields & " from " & cFileName & " where list# >= " & Wrklistno & " Order by list#"
    Else
  ...
  Public Function AutoGenKey() As Integer
    ...
    RecordNotFound = False
    StrSQL = "Select top 1 * from " & cFileName & " order by list# desc"
    Try
      Conn = MyDBConn.Open
      objCommand = New SqlCommand(StrSQL, Conn)
      'Fill the dataset with the data
      ...
      If ds.Tables(0).Rows.Count = 0 Then
        NextKey = 1
      Else
        NextKey = ds.Tables(0).Rows(0).Item("list#") + 1
      End If
    ...
    Return NextKey
  End Function
  Public Sub ClearBanks()
    Dim Conn As SqlConnection
    Dim objCommand As SqlCommand
    Dim Result As Integer

    StrSQL = "UPDATE " & cFileName & " SET bkcd='', bksv=''"
    Conn = MyDBConn.Open
    objCommand = New SqlCommand(StrSQL, Conn)
