...
#Region "Methods: File Access Routines"
  '   NOTE this  is joining 2 files together in these routines TXMVD and TXMSRP
  Public Function GetQry(ByVal WrkSort As String, ByVal WrkQry As String, ByVal NumRecs As Long) As DataSet
    ...
    StrSQL = "Select " & WrkTop & " TXMVD.*,ISNULL(TXMSRP.OVMSRP, 0) AS OVMSRP, ISNULL(TXMSRP.OVSOURCE, '') AS OVSOURCE,ISNULL(TXMSRP.COMPLETE, '') AS COMPLETE from TXMVD LEFT JOIN TXMSRP ON TXMVD.vinno = TXMSRP.vinno"
  ...
  Public Sub OpenQry(ByVal WrkSort As String, ByVal WrkQry As String)
    ...
    StrSQL = "Select TXMVD.*,ISNULL(TXMSRP.OVMSRP, 0) AS OVMSRP, ISNULL(TXMSRP.OVSOURCE, '') AS OVSOURCE,ISNULL(TXMSRP.COMPLETE, '') AS COMPLETE from TXMVD LEFT JOIN TXMSRP ON TXMVD.vinno = TXMSRP.vinno"
    If WrkQry <> String.Empty Then
      StrSQL = StrSQL & " where " & WrkQry
    End If
    If WrkSort <> String.Empty Then
...
Dim mLOC as String
  ...
  Dim mOVMSRP As String
  Public Property _OVMSRP As String
    Get
      Return mOVMSRP
    End Get
    Set(ByVal value As String)
