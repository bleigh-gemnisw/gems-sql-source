...
Public Function GetViewGrid(ByVal WrkYear As Integer, ByVal WrkAcct As String, _
 ByVal NumRecs As Integer) As DataSet
  ...
  If NumRecs > 0 Then
    WrkTop = "TOP " & NumRecs & " "
  End If
  StrSQL = "Select " & WrkTop & "ayear,aracct,arname from " & cFileName _
    & " where ayear=" & WrkYear & " and aracct >='" & WrkAcct & "' or " _
    & " ayear>" & WrkYear & " order by ayear, aracct"
  Try
    Conn = MyDBConn.Open
...
Public Function GetRecordCount() As Integer
  ...
  RecordNotFound = False
  StrSQL = "Select count(aracct) from " & cFileName
  Try
    Conn = MyDBConn.Open
    objCommand = New SqlCommand(StrSQL, Conn)
    'Fill the dataset with the data
