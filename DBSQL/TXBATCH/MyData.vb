...
#Region "Methods: File Access Routines"
  ...
  Public Function AutoGenKey(ByVal WrkBatchCd As String, ByVal WrkBchno As Integer) As Integer
    ...
    StrSQL = "Select * from " & cFileName & " where jbtchc='" & WrkBatchCd & "' and jbatch=" & WrkBchno &
     " order by jseqno desc"
    Try
      Conn = MyDBConn.Open
      ...
      If ds.Tables(0).Rows.Count = 0 Then
        NextKey = 1
      Else
        NextKey = ds.Tables(0).Rows(0).Item("jseqno") + 1
      End If
      objCommand = Nothing
      ds.Clear()
      ds = Nothing
    ...
    Return NextKey
  End Function
  Public Function CalcBatchSubTotals(ByVal WrkBatchCd As String, ByVal WrkBchno As Integer) As Array
    Dim Conn As SqlConnection
    Dim objCommand As SqlCommand
    Dim ds As DataSet = New DataSet
    ...
    WrkAmount(1) = 0
    WrkAmount(2) = 0
    StrSQL = "Select cash, [check],credit,tbl from " & cFileName _
    & " where jbatch=" & WrkBchno & " and jstat<>'V' order by jseqno desc"
    Try
      Conn = MyDBConn.Open
      objCommand = New SqlCommand(StrSQL, Conn)
      'Fill the dataset with the data
      ...
      For I = 0 To ds.Tables(0).Rows.Count - 1
        If ds.Tables(0).Rows(I).Item("tbl") <> 0 Then Exit For
        WrkAmount(0) = WrkAmount(0) + ds.Tables(0).Rows(I).Item("cash")
        WrkAmount(1) = WrkAmount(1) + ds.Tables(0).Rows(I).Item("check")
        WrkAmount(2) = WrkAmount(2) + ds.Tables(0).Rows(I).Item("credit")
      Next
      objCommand = Nothing
      ds = Nothing
      Conn.Close()
  ...
  Public Sub GetOneRecordP(ByVal WrkBatchCd As String, WrkBchno As Integer, ByVal WrkSeqno As Integer)
    ...
    End Try
  End Sub
  Public Function PosData(ByVal WrkBatchCd As String, WrkBchno As Integer, ByVal WrkSeqno As Integer,
   ByVal NumRecs As Integer) As DataSet
    ...
    If NumRecs > 0 Then
      WrkTop = "TOP " & NumRecs & " "
    End If
    StrSQL = "Select " & WrkTop & "* from " & cFileName & " where jbtchc='" & WrkBatchCd &
     "' and jbatch=" & WrkBchno & " and jseqno>=" & WrkSeqno & " order by jseqno"
    Conn = MyDBConn.Open
    objCommand = New SqlCommand(StrSQL, Conn)
