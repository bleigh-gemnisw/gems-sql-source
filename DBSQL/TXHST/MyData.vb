...
#Region "Methods: File Access Routines"
  ...
  Public Sub GetOneRecordP(ByVal Wrkrecid As Integer)
    ...
    End Try
  End Sub
  Public Sub CheckifLienReleased(ByVal Wrklist As Integer, ByVal wrkType As String, ByVal wrkyear As Integer)
    Dim Conn As SqlConnection
    Dim objCommand As SqlCommand
    Dim ds As DataSet = New DataSet

    RecordNotFound = False
    StrSQL = "Select * from " & cFileName & " where rcode = 'I' AND comm = 'Lien Released' AND list# = " & Wrklist & " AND year = " & wrkyear & " AND type = '" & wrkType & "'"
    Try
      Conn = MyDBConn.Open
      objCommand = New SqlCommand(StrSQL, Conn)
      'Fill the dataset with the data
