...
#Region "Methods: File Access Routines"
  Public Sub ClearFields()
    ...
    _RCODE = String.Empty
  End Sub
  Public Sub GetOneRecordP(ByVal Wrkcuacct As Integer)
    Dim Conn As SqlConnection
    Dim objCommand As SqlCommand
    Dim ds As DataSet = New DataSet

    RecordNotFound = False
    StrSQL = "Select * from " & cFileName & " where cuacct = " & Wrkcuacct
    Try
      Conn = MyDBConn.Open
    ...
    End Try
  End Sub
  Public Function PosData(ByVal Wrkcuacct As Integer) As DataSet
    Dim Conn As SqlConnection
    Dim objCommand As SqlCommand
    Dim ds As DataSet = New DataSet

    StrSQL = "Select * from " & cFileName & " where cuacct >= " & Wrkcuacct & " Order by cuacct"
    Conn = MyDBConn.Open
    objCommand = New SqlCommand(StrSQL, Conn)
  ...
  Public Sub AddOneRecordP()
    ...
    ds = Nothing
  End Sub
  Public Function AutoGenKey() As Integer    ' added function 5-6-26  ken   
    ...
    RecordNotFound = False
    StrSQL = "Select top 1 * from " & cFileName & " order by cuacct desc"
    Try
      Conn = MyDBConn.Open
      objCommand = New SqlCommand(StrSQL, Conn)
      'Fill the dataset with the data
      ...
      If ds.Tables(0).Rows.Count = 0 Then
        NextKey = 1
      Else
        NextKey = ds.Tables(0).Rows(0).Item("cuacct") + 1
      End If
    ...
    Return NextKey
  End Function
  Public Sub ChangeMeterSize(ByVal WrkOldCode As String, ByVal WrkNewCode As String)
    Dim Conn As SqlConnection
    Dim objCommand As SqlCommand
    Dim Result As Integer

    StrSQL = "UPDATE " & cFileName & " SET cumsiz='" & WrkNewCode & "' WHERE cumsiz = '" & WrkOldCode & "'"
