...
#Region "Methods: File Access Routines"
  Public Function GetViewbyList(ByVal WrkList As Integer, ByVal WrkYear As Integer, ByVal WrkType As String,
   ByVal WrkDate As Integer, ByVal NumRecs As Integer) As DataSet
    ...
    StrSQL = "Select " & WrkTop & "list#,year,type,pdate,cdate,pamt,iamt,lamt,pcamt,pencd,adjcd,batcha,batchn,corc,cash," _
    & "[check], credit,ref,comm,rcode from " & cFileName _
    & " where list# =" & WrkList & " And type ='" & WrkType & "' and year =" & WrkYear & " and pdate>=" & WrkDate _
    & " order by list#,year,type,pdate,chdate,chtime"
    Try
      Conn = MyDBConn.Open
  ...
  Public Function GetViewbyListQ1(ByVal WrkList As Integer, ByVal WrkYear As Integer, ByVal WrkType As String,
   ByVal WrkDate As Integer, ByVal NumRecs As Integer) As DataSet
    Dim Conn As SqlConnection
    Dim objCommand As SqlCommand
    Dim ds As DataSet = New DataSet
    ...
    If NumRecs > 0 Then
      WrkTop = "TOP " & NumRecs & " "
    End If
    StrSQL = "Select " & WrkTop & "rcode,list#,year,type,pamt,pdate,adjcd from " & cFileName _
    & " where list# =" & WrkList & " And type ='" & WrkType & "' and year =" & WrkYear & " and pdate>=" & WrkDate _
    & " order by list#,year,type,pdate,chdate,chtime"
    Try
      Conn = MyDBConn.Open
    ...
    End Try
  End Function
  Public Function GetViewDscList(ByVal WrkList As Integer, ByVal WrkYear As Integer, ByVal WrkType As String,
   ByVal WrkDate As Integer, ByVal NumRecs As Integer) As DataSet
    Dim Conn As SqlConnection
    Dim objCommand As SqlCommand
    Dim ds As DataSet = New DataSet
    ...
    StrSQL = "Select " & WrkTop & "list#,year,type,pdate,cdate,pamt,iamt,lamt,pcamt,adjcd,pencd,batcha,batchn,corc,cash," _
    & "[check], credit,ref,comm,rcode from " & cFileName _
    & " where list# =" & WrkList & " And type ='" & WrkType & "' and year =" & WrkYear & " and pdate<=" & WrkDate _
    & " order by list#,year,type,pdate desc,chdate desc,chtime desc"
    Try
      Conn = MyDBConn.Open
      objCommand = New SqlCommand(StrSQL, Conn)
      'Fill the dataset with the data
  ...
  Public Function ReplaceDS(ByVal ds As DataSet) As DataSet
    ...
    With myTable
      ...
      .Columns.Add("rcode", Type.GetType("System.String"))
    End With
    ...
    For I = 0 To ds.Tables(0).Rows.Count - 1
      With ds.Tables(0).Rows(I)
        ...
        dr.Item("corc") = .Item("corc")
        dr.Item("ref") = .Item("ref")
        dr.Item("comm") = .Item("comm")
        dr.Item("rcode") = .Item("rcode")
  ...
  Public Function GetDBDateInt(ByVal DateIn As Integer) As Integer
    ...
    Return WrkDate
  End Function
  Public Sub SetRange(ByVal WrkListNo As Integer, ByVal WrkYear As Integer, ByVal WrkType As String, ByVal WrkDate As Integer, WrkDescending As Boolean)
    Dim objCommand As SqlCommand
    Dim WrkPdate As String
    Dim StrDesc As String

    RecordNotFound = False
    IsEOF = False
    If WrkDescending Then
      WrkPdate = " pdate<=" & WrkDate
      StrDesc = " Desc"
    Else
      WrkPdate = " pdate>=" & WrkDate
      StrDesc = ""
    End If
    StrSQL = "Select * from " & cFileName & " where list#=" & WrkListNo & " and year=" & WrkYear _
     & " and type='" & WrkType & "' and " & WrkPdate & " order by pdate" & StrDesc
