...
Public Class MyData
  ...
  Const cFileName As String = "TXREALC"
#Region "Constructors"
...
#Region "Methods: File Access Routines"
  Public Function GetViewbyLoc(ByVal Wrkloc As String, ByVal wrklocno As String, NumRecs As Integer) As DataSet
    ...
    & " where concat(RTrim(loc), RTrim(loc#)) >= '" & wrkloclocno & "' order by loc,loc#,unit#"
    ...
    End Try
  End Function
  Public Function GetViewgrid(ByVal Wrkloc As String, ByVal wrklocno As String, NumRecs As Integer) As DataSet
    Dim Conn As SqlConnection
    Dim objCommand As SqlCommand
    Dim ds As DataSet = New DataSet
    ...
    If NumRecs > 0 Then
      WrkTop = "TOP " & NumRecs & " "
    End If
    StrSQL = "Select " & WrkTop & " list#,loc#,loc,unit#,name,sname from " & cFileName _
   & " where concat(RTrim(loc), RTrim(loc#)) >= '" & wrkloclocno & "' order by loc,loc#,unit#"
    Try
      Conn = MyDBConn.Open
      objCommand = New SqlCommand(StrSQL, Conn)
