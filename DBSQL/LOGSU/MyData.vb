...
Public Class MyData
  ...
  Const cFileName As String = "LOGSU"
  Const cFileName2 As String = "TXSUPP"
#Region "Constructors"
...
#Region "Methods: File Access Routines"
  ...
  Public Function GetAllList(ByVal WrkListNo As Integer) As DataSet
    ...
    If ds.Tables(0).Rows.Count > 0 Then
      StrSQL = "Select * from " & cFileName2 & " where list#=" & WrkListNo
      Conn = MyDBConn.Open
      objCommand = New SqlCommand(StrSQL, Conn)
      da = New SqlDataAdapter
      da.SelectCommand = objCommand
  ...
  Public Function PosData(ByVal WrkListNo As Integer, ByVal WrkLogdte As Integer, ByVal WrkLogTim As Integer,
   ByVal NumRecs As Long) As DataSet
    ...
    End If
    RecordNotFound = False
    StrSQL = "Select " & WrkTop & " * from " & cFileName & " where list#=" & WrkListNo &
   " and logdte=" & WrkLogdte & " and logtim>=" & WrkLogTim &
   " or list#=" & WrkListNo & "and logdte>" & WrkLogdte &
   " or list#>" & WrkListNo
    Conn = MyDBConn.Open
    objCommand = New SqlCommand(StrSQL, Conn)
