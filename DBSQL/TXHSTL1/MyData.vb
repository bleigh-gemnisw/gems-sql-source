...
#End Region
  Public Function GetbyList(ByVal ListNo As Integer, ByVal Year As Integer,
 ...
 ByVal Type As String, ByVal PDate As Integer, ByVal NumRecs As Integer) As DataSet
    Dim Conn As SqlConnection
    Dim objCommand As SqlCommand
    Dim ds As DataSet = New DataSet
    ...
    End If
    RecordNotFound = False
    StrSQL = "Select " & WrkTop & " list#, type, year, pdate,pamt,iamt,pcamt,lamt," &
   "adjcd,batcha,batchn,corc,ref,comm,rcode from " & cFileName _
    & " where list#=" & ListNo & " and type='" & Type & "' and year=" & Year &
    " order by pdate"
    Try
      Conn = MyDBConn.Open
  ...
  Public Function GetDBDateInt(ByVal DateIn As Integer) As Integer
    ...
    Return WrkDate
  End Function
  Public Sub SetRange(ByVal ListNo As Integer, ByVal Year As Integer,
 ByVal Type As String, ByVal PDate As Integer, ByVal Desc As Boolean)
    Dim objCommand As SqlCommand
    ...
    " where list#=" & ListNo & " and type='" & Type & "' and year=" & Year
    If PDate > 0 Then
      StrSQL = StrSQL & " and pdate" & WrkCompare & PDate
    End If
    StrSQL = StrSQL & " order by pdate"
    If Desc Then
      StrSQL = StrSQL & " desc"
    End If
  ...
  '------------------------ 05/30/25 added i nattempt to speeed up balance sheet txe08
  Public Sub SetRangeLite(ByVal ListNo As Integer, ByVal Year As Integer, ByVal Type As String, ByVal PDate As Integer, ByVal Desc As Boolean)
    Dim objCommand As SqlCommand
    Dim WrkCompare As String
    RecordNotFound = False
    IsEOF = False
    ...
    Else
      WrkCompare = "<="
    End If
    StrSQL = "Select PDATE, PAMT, LAMT, IAMT, PCAMT, RCODE, ADJCD, BATCHA, SUSCD from " & cFileName &
    " where list#=" & ListNo & " and type='" & Type & "' and year=" & Year
    If PDate > 0 Then
      StrSQL = StrSQL & " and pdate" & WrkCompare & PDate
    End If
