...
#Region "Methods: File Access Routines"
  ...
  Public Function GetLatestAll() As DataSet
    ...
    StrSQL = "Select d.* from " & cFileName & " d " &
    "inner join (select ddacct, ddmeter, max(dddate) as maxdate " &
    "from " & cFileName & " group by ddacct, ddmeter) x " &
    "on d.ddacct=x.ddacct and d.ddmeter=x.ddmeter and d.dddate=x.maxdate " &
  ...
  Public Function GetLastbyDate(ByVal WrkListNo As Integer, ByVal WrkMeterNo As String,
    ...
    End Try
  End Function
  Public Function GetLastDiff(ByVal WrkListNo As Integer, ByVal WrkMeterNo As String, ByVal WrkDate As Integer) As DataSet
    Dim Conn As SqlConnection
    Dim objCommand As SqlCommand
    Dim ds As DataSet = New DataSet
    ...
    RecordNotFound = False
    ErrMsg = String.Empty
    StrSQL = "Select TOP 1 * from " & cFileName & " where ddacct=" & WrkListNo &
      " and ddmeter='" & Trim(WrkMeterNo) & "' and dddate<=" & WrkDate & " and dddiff>0" & " order by dddate desc"
    Try
      Conn = MyDBConn.Open
      objCommand = New SqlCommand(StrSQL, Conn)
      da = New SqlDataAdapter
  ...
  Public Sub RecalcAllDifferences(ByVal WrkListNo As Integer, ByVal WrkMeterNo As String)
    ...
    For Each Row As DataRow In ds.Tables(0).Rows
      ...
      If FirstRecord Then
        'First reading starts from a zero base.
        ThisDiff = ThisReading
      Else
